...
WVQA payloads are still partial at 1608/1955 payloads, while materialized runtime
packs now cover 66/66 archives. The VQA runtime pack build report makes the
staging boundary explicit: `replacement_entries=1608/1955`,
`applied_replacements=1563/1955`, `deferred_replacements=45`,
`missing_replacements=347`, and `output_archives=66/66`, so it writes 66
partial runtime MIX files and keeps the global requirement in `gap`. `L20_BBI.MIX`
uses 306/351 currently available replacements and defers 45 more to stay below
...
1920x1080 `FORM/WVQA` main batch plus a 3-entry `--missing-only` incremental
batch, validates 73647/73647 decoded frames across current payload coverage,
records `exact_block_ratio=0.917133`, and installs them under
...
66/66 exact layout-preserving MIX roundtrips, 1608/1955 encoded WVQA replacement
...
Current result: `gap`, with `replacement_entries=1608/1955`,
`applied_replacements=1563/1955`, `deferred_replacements=45`,
`missing_replacements=347`, and `output_archives=66/66`. Sixty-six partial runtime
VQA MIX files are written. `L20_BBI.MIX` applies 306/351 available replacements
and defers 45 entries so the rebuilt body stays under the 4294967295-byte MIX
...
Current result: `pass`, with 73647/73647 decoded frames validated at
1920x1080, 22603620816 payload bytes, 4000/4000 max vectors used,
`exact_block_ratio=0.917133`, and `changed_pixel_ratio=0.036322`. The writer
