...
WVQA payloads are still partial at 1848/1955 payloads, while materialized runtime
packs now cover 66/66 archives. The VQA runtime pack build report makes the
staging boundary explicit: `replacement_entries=1848/1955`,
`applied_replacements=1797/1955`, `deferred_replacements=51`,
`missing_replacements=107`, and `output_archives=66/66`, so it writes 66
...
24-entry, 26-entry, 28-entry, and 30-entry `--missing-only` incremental batches. The thirteen latest
...
the latest incremental batch validates 8644/8644 additional decoded frames. The
runtime feasibility report aggregates those writer outputs as 110180/110180
validated frames with `exact_block_ratio=0.889743` and installs them under
...
66/66 exact layout-preserving MIX roundtrips, 1848/1955 encoded WVQA replacement
...
Current result: `gap`, with `replacement_entries=1848/1955`,
`applied_replacements=1797/1955`, `deferred_replacements=51`,
`missing_replacements=107`, and `output_archives=66/66`. Sixty-six partial runtime
...
incremental non-`L20_BBI` batch result is also `pass`, with 8644/8644 decoded
frames validated, 2778072854 payload bytes, `exact_block_ratio=0.818955`, and
`changed_pixel_ratio=0.101140`. The feasibility report now aggregates accepted
writer outputs as 110180/110180 validated frames, 34216006420 payload bytes,
`exact_block_ratio=0.889743`, and `changed_pixel_ratio=0.051568`. The writer
