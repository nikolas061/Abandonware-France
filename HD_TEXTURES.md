...
WVQA payloads are still partial at 1628/1955 payloads, while materialized runtime
packs now cover 66/66 archives. The VQA runtime pack build report makes the
staging boundary explicit: `replacement_entries=1628/1955`,
`applied_replacements=1577/1955`, `deferred_replacements=51`,
`missing_replacements=327`, and `output_archives=66/66`, so it writes 66
...
1920x1080 `FORM/WVQA` main batch plus 3-entry, 6-entry, 6-entry, and 8-entry
`--missing-only` incremental batches. The two latest batches exclude `L20_BBI`
...
the latest incremental batch validates 758/758 additional decoded frames. The
runtime feasibility report aggregates those writer outputs as 75774/75774
validated frames with `exact_block_ratio=0.916616` and installs them under
...
66/66 exact layout-preserving MIX roundtrips, 1628/1955 encoded WVQA replacement
...
Current result: `gap`, with `replacement_entries=1628/1955`,
`applied_replacements=1577/1955`, `deferred_replacements=51`,
`missing_replacements=327`, and `output_archives=66/66`. Sixty-six partial runtime
...
incremental non-`L20_BBI` batch result is also `pass`, with 758/758 decoded
frames validated, 239971804 payload bytes, `exact_block_ratio=0.867857`, and
`changed_pixel_ratio=0.058637`. The feasibility report now aggregates accepted
writer outputs as 75774/75774 validated frames, 23258024662 payload bytes,
`exact_block_ratio=0.916616`, and `changed_pixel_ratio=0.036562`. The writer
