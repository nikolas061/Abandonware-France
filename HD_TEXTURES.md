...
WVQA payloads are still partial at 1790/1955 payloads, while materialized runtime
packs now cover 66/66 archives. The VQA runtime pack build report makes the
staging boundary explicit: `replacement_entries=1790/1955`,
`applied_replacements=1739/1955`, `deferred_replacements=51`,
`missing_replacements=165`, and `output_archives=66/66`, so it writes 66
...
24-entry, and 26-entry `--missing-only` incremental batches. The eleven latest
...
the latest incremental batch validates 4083/4083 additional decoded frames. The
runtime feasibility report aggregates those writer outputs as 95966/95966
validated frames with `exact_block_ratio=0.904644` and installs them under
...
66/66 exact layout-preserving MIX roundtrips, 1790/1955 encoded WVQA replacement
...
Current result: `gap`, with `replacement_entries=1790/1955`,
`applied_replacements=1739/1955`, `deferred_replacements=51`,
`missing_replacements=165`, and `output_archives=66/66`. Sixty-six partial runtime
...
incremental non-`L20_BBI` batch result is also `pass`, with 4083/4083 decoded
frames validated, 1304794704 payload bytes, `exact_block_ratio=0.846463`, and
`changed_pixel_ratio=0.085418`. The feasibility report now aggregates accepted
writer outputs as 95966/95966 validated frames, 29696342212 payload bytes,
`exact_block_ratio=0.904644`, and `changed_pixel_ratio=0.043254`. The writer
