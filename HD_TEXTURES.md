...
(`sample_saved_ratio=0.704964`). The LCW compact payload materializer writes 86
complete compact deferred payloads under 32 MB in `L20_BBI.MIX` from the
previous compact report plus the base and current compact oversize budgets:
7089 frames, 14178 chunks recompressed with 0 roundtrip failures,
2097781272 -> 463397428 bytes, and `saved_ratio=0.779101`. The runtime pack
...
uses those compact payloads, reports `overlay_replacements=86`, applies 369/407
replacements, defers 38, writes `output_bytes=4288135130`, and leaves
`required_reduction_bytes=2307283761` in the matching sample oversize budget.
...
Current result: `pass`, with 86/86 selected deferred replacements written,
7089 frames, 14178 recompressed chunks, 0 chunk roundtrip failures,
`original_payload_bytes=2097781272`, `compact_payload_bytes=463397428`, and
`saved_bytes=1634383844` (`saved_ratio=0.779101`). The compact payloads are
...
`replacement_entries=407/407`, `overlay_replacements=86`,
`applied_replacements=369/407`, `deferred_replacements=38`,
`missing_replacements=0`, `output_archives=1/1`, and
`output_bytes=4288135130`. The 86 compact rows now resolve to compact overlay
...
Current compact sample budget: `gap`, with 1 oversized archive, 38 deferred
replacements, `headroom_bytes=6837775`,
`deferred_replacement_bytes=2411549430`, and
`required_reduction_bytes=2307283761` remaining for `L20_BBI.MIX`.
