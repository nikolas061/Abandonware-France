...
WVQA payloads are still partial at 1718/1955 payloads, while materialized runtime
packs now cover 66/66 archives. The VQA runtime pack build report makes the
staging boundary explicit: `replacement_entries=1718/1955`,
`applied_replacements=1667/1955`, `deferred_replacements=51`,
`missing_replacements=237`, and `output_archives=66/66`, so it writes 66
...
10-entry, 12-entry, 14-entry, 16-entry, 18-entry, and 20-entry `--missing-only`
incremental batches. The eight latest
...
the latest incremental batch validates 2439/2439 additional decoded frames. The
runtime feasibility report aggregates those writer outputs as 85547/85547
validated frames with `exact_block_ratio=0.909090` and installs them under
...
66/66 exact layout-preserving MIX roundtrips, 1718/1955 encoded WVQA replacement
...
Current result: `gap`, with `replacement_entries=1718/1955`,
`applied_replacements=1667/1955`, `deferred_replacements=51`,
`missing_replacements=237`, and `output_archives=66/66`. Sixty-six partial runtime
...
incremental non-`L20_BBI` batch result is also `pass`, with 2439/2439 decoded
frames validated, 768501576 payload bytes, `exact_block_ratio=0.852262`, and
`changed_pixel_ratio=0.065401`. The feasibility report now aggregates accepted
writer outputs as 85547/85547 validated frames, 26343630934 payload bytes,
`exact_block_ratio=0.909090`, and `changed_pixel_ratio=0.039919`. The writer
