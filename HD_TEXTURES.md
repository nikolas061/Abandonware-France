...
WVQA payloads are still partial at 1740/1955 payloads, while materialized runtime
packs now cover 66/66 archives. The VQA runtime pack build report makes the
staging boundary explicit: `replacement_entries=1740/1955`,
`applied_replacements=1689/1955`, `deferred_replacements=51`,
`missing_replacements=215`, and `output_archives=66/66`, so it writes 66
...
10-entry, 12-entry, 14-entry, 16-entry, 18-entry, 20-entry, and 22-entry
`--missing-only` incremental batches. The nine latest
...
the latest incremental batch validates 2881/2881 additional decoded frames. The
runtime feasibility report aggregates those writer outputs as 88428/88428
validated frames with `exact_block_ratio=0.909739` and installs them under
...
66/66 exact layout-preserving MIX roundtrips, 1740/1955 encoded WVQA replacement
...
Current result: `gap`, with `replacement_entries=1740/1955`,
`applied_replacements=1689/1955`, `deferred_replacements=51`,
`missing_replacements=215`, and `output_archives=66/66`. Sixty-six partial runtime
...
incremental non-`L20_BBI` batch result is also `pass`, with 2881/2881 decoded
frames validated, 941452254 payload bytes, `exact_block_ratio=0.929032`, and
`changed_pixel_ratio=0.052056`. The feasibility report now aggregates accepted
writer outputs as 88428/88428 validated frames, 27285083188 payload bytes,
`exact_block_ratio=0.909739`, and `changed_pixel_ratio=0.040314`. The writer
