...
WVQA payloads are still partial at 1764/1955 payloads, while materialized runtime
packs now cover 66/66 archives. The VQA runtime pack build report makes the
staging boundary explicit: `replacement_entries=1764/1955`,
`applied_replacements=1713/1955`, `deferred_replacements=51`,
`missing_replacements=191`, and `output_archives=66/66`, so it writes 66
...
24-entry `--missing-only` incremental batches. The ten latest
...
the latest incremental batch validates 3455/3455 additional decoded frames. The
runtime feasibility report aggregates those writer outputs as 91883/91883
validated frames with `exact_block_ratio=0.907230` and installs them under
...
66/66 exact layout-preserving MIX roundtrips, 1764/1955 encoded WVQA replacement
...
Current result: `gap`, with `replacement_entries=1764/1955`,
`applied_replacements=1713/1955`, `deferred_replacements=51`,
`missing_replacements=191`, and `output_archives=66/66`. Sixty-six partial runtime
...
incremental non-`L20_BBI` batch result is also `pass`, with 3455/3455 decoded
frames validated, 1106464320 payload bytes, `exact_block_ratio=0.842998`, and
`changed_pixel_ratio=0.068659`. The feasibility report now aggregates accepted
writer outputs as 91883/91883 validated frames, 28391547508 payload bytes,
`exact_block_ratio=0.907230`, and `changed_pixel_ratio=0.041380`. The writer
