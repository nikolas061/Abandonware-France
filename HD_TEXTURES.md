...
(`sample_saved_ratio=0.704964`). The LCW compact payload materializer writes 69
complete compact deferred payloads under 30 MB in `L20_BBI.MIX` from the
previous compact report plus the base and current compact oversize budgets:
5340 frames, 10680 chunks recompressed with 0 roundtrip failures,
1580393880 -> 350418414 bytes, and `saved_ratio=0.778271`. The runtime pack
...
uses those compact payloads, reports `overlay_replacements=69`, applies 358/407
replacements, defers 49, writes `output_bytes=4292326344`, and leaves
`required_reduction_bytes=2711692139` in the matching sample oversize budget.
...
Current result: `pass`, with 69/69 selected deferred replacements written,
5340 frames, 10680 recompressed chunks, 0 chunk roundtrip failures,
`original_payload_bytes=1580393880`, `compact_payload_bytes=350418414`, and
`saved_bytes=1229975466` (`saved_ratio=0.778271`). The compact payloads are
...
`replacement_entries=407/407`, `overlay_replacements=69`,
`applied_replacements=358/407`, `deferred_replacements=49`,
`missing_replacements=0`, `output_archives=1/1`, and
`output_bytes=4292326344`. The 69 compact rows now resolve to compact overlay
...
Current compact sample budget: `gap`, with 1 oversized archive, 49 deferred
replacements, `headroom_bytes=2646561`,
`deferred_replacement_bytes=2820366678`, and
`required_reduction_bytes=2711692139` remaining for `L20_BBI.MIX`.
