...
WVQA payloads are still partial at 1818/1955 payloads, while materialized runtime
packs now cover 66/66 archives. The VQA runtime pack build report makes the
staging boundary explicit: `replacement_entries=1818/1955`,
`applied_replacements=1767/1955`, `deferred_replacements=51`,
`missing_replacements=137`, and `output_archives=66/66`, so it writes 66
...
24-entry, 26-entry, and 28-entry `--missing-only` incremental batches. The twelve latest
...
the latest incremental batch validates 5570/5570 additional decoded frames. The
runtime feasibility report aggregates those writer outputs as 101536/101536
validated frames with `exact_block_ratio=0.895769` and installs them under
...
66/66 exact layout-preserving MIX roundtrips, 1818/1955 encoded WVQA replacement
...
Current result: `gap`, with `replacement_entries=1818/1955`,
`applied_replacements=1767/1955`, `deferred_replacements=51`,
`missing_replacements=137`, and `output_archives=66/66`. Sixty-six partial runtime
...
incremental non-`L20_BBI` batch result is also `pass`, with 5570/5570 decoded
frames validated, 1741591354 payload bytes, `exact_block_ratio=0.742858`, and
`changed_pixel_ratio=0.117889`. The feasibility report now aggregates accepted
writer outputs as 101536/101536 validated frames, 31437933566 payload bytes,
`exact_block_ratio=0.895769`, and `changed_pixel_ratio=0.047348`. The writer
