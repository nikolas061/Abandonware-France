...
WVQA payloads are still partial at 1638/1955 payloads, while materialized runtime
packs now cover 66/66 archives. The VQA runtime pack build report makes the
staging boundary explicit: `replacement_entries=1638/1955`,
`applied_replacements=1587/1955`, `deferred_replacements=51`,
`missing_replacements=317`, and `output_archives=66/66`, so it writes 66
...
1920x1080 `FORM/WVQA` main batch plus 3-entry, 6-entry, 6-entry, 8-entry, and
10-entry `--missing-only` incremental batches. The three latest batches exclude `L20_BBI`
...
the latest incremental batch validates 975/975 additional decoded frames. The
runtime feasibility report aggregates those writer outputs as 76749/76749
validated frames with `exact_block_ratio=0.916133` and installs them under
...
66/66 exact layout-preserving MIX roundtrips, 1638/1955 encoded WVQA replacement
...
Current result: `gap`, with `replacement_entries=1638/1955`,
`applied_replacements=1587/1955`, `deferred_replacements=51`,
`missing_replacements=317`, and `output_archives=66/66`. Sixty-six partial runtime
...
incremental non-`L20_BBI` batch result is also `pass`, with 975/975 decoded
frames validated, 311389186 payload bytes, `exact_block_ratio=0.878592`, and
`changed_pixel_ratio=0.052622`. The feasibility report now aggregates accepted
writer outputs as 76749/76749 validated frames, 23569413848 payload bytes,
`exact_block_ratio=0.916133`, and `changed_pixel_ratio=0.036766`. The writer
