#!/usr/bin/env python3
"""Promote reviewed guarded source bytes into fixture buffers."""
...
DEFAULT_OUTPUT = Path("output/tex_gradient_sequence_high_safe_low_exception_source_byte_guard_promoted_replay")
...
DEFAULT_TARGETS = Path("output/tex_gradient_sequence_high_safe_low_exception_source_byte_guard/targets.csv")
...
def build_rows(
    *,
    ...
    for base_fixture in sorted(base_fixture_rows, key=lambda row: fixture_sort_key(archive_key(row))):
        ...
        source_mask = bytearray(len(expected))
        stats: Counter[str] = Counter()

        for target in target_groups.get(key, []):
            target_issues: list[str] = []
            offset = int_value(target, "source_offset", int_value(target, "target_offset", -1))
            ...
            source_overlap = overlap_bytes(source_mask, offset, offset + 1)
            ...
            if predicted is not None and expected_value is not None and predicted != expected_value:
                target_issues.append("source_would_write_false_byte")
            ...
            if source_overlap:
                target_issues.append("source_overlap")

            false_bytes = 1 if "source_would_write_false_byte" in target_issues else 0
            added_bytes = 0
            exact_bytes = 0
            skipped_known = 0
            skipped_rejected = 0
            if not target_issues:
                if base_overlap:
                    ...
                    source_mask[offset] = 0xFF
                    added_bytes = 1
                    exact_bytes = 1
            else:
                issue_rows += 1

            stats["source_candidate_bytes"] += 1
            stats["source_added_bytes"] += added_bytes
            stats["source_exact_bytes"] += exact_bytes
            stats["source_false_bytes"] += false_bytes
        ...
        decoded_path = fixture_output_dir / f"{stem}_decoded_source_byte_guard.bin"
        known_mask_path = fixture_output_dir / f"{stem}_known_mask.bin"
        source_mask_path = fixture_output_dir / f"{stem}_source_byte_guard_mask.bin"
        decoded_path.write_bytes(decoded)
        known_mask_path.write_bytes(known_mask)
        source_mask_path.write_bytes(source_mask)
        native_preview_path = native_preview_dir / f"{stem}_source_byte_guard.png"
        fullhd_preview_path = fullhd_preview_dir / f"{stem}_source_byte_guard_fullhd.png"
        fullhd_width, fullhd_height = render_preview(
            expected=expected,
            decoded=bytes(decoded),
            known_mask=bytes(known_mask),
            risk_mask=bytes(source_mask),
        ...
        output_fixture_rows.append(
            {
                ...
                "source_target_rows": str(len(target_groups.get(key, []))),
                "source_candidate_bytes": str(stats["source_candidate_bytes"]),
                "source_added_bytes": str(stats["source_added_bytes"]),
                "source_exact_bytes": str(stats["source_exact_bytes"]),
                "source_false_bytes": str(stats["source_false_bytes"]),
    ...
    source_added = sum(int_value(row, "source_added_bytes") for row in output_fixture_rows)
    source_false = sum(int_value(row, "source_false_bytes") for row in output_fixture_rows)
    summary = {
        "scope": "total",
        "fixture_rows": str(len(output_fixture_rows)),
        "target_rows": str(sum(len(rows) for rows in target_groups.values())),
        "promoted_rows": str(sum(1 for row in promotion_rows if int_value(row, "source_added_bytes") > 0)),
        "base_clean_bytes": str(sum(int_value(row, "base_clean_bytes") for row in output_fixture_rows)),
        "source_candidate_bytes": str(sum(int_value(row, "source_candidate_bytes") for row in output_fixture_rows)),
        "source_added_bytes": str(source_added),
        "source_exact_bytes": str(sum(int_value(row, "source_exact_bytes") for row in output_fixture_rows)),
        "source_false_bytes": str(source_false),
        ...
        "promotion_ready_bytes": str(source_added if issue_rows == 0 and source_false == 0 else 0),
...
<div class="grid">
  <div class="box"><div class="num">{summary['source_added_bytes']}</div><div class="muted">source added bytes</div></div>
  <div class="box"><div class="num">{summary['promoted_rows']}/{summary['target_rows']}</div><div class="muted">promoted targets</div></div>
  <div class="box"><div class="num">{summary['source_false_bytes']}</div><div class="muted">false bytes</div></div>
...
<script type="application/json" id="source-byte-guard-promoted-replay-data">{html.escape(data_json)}</script>
</body>
</html>
"""
...
def main() -> None:
    ...
    parser.add_argument("--title", default="Lands of Lore II .tex Source Byte Guard Promoted Replay")
