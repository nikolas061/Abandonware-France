#!/usr/bin/env python3
"""Score source-side selectors for unresolved internal len64 zero targets."""
...
DEFAULT_OUTPUT = Path("output/tex_gap_decoder_len64_selector_probe")
DEFAULT_TARGETS = Path("output/tex_gap_decoder_len64_source_probe/targets.csv")
DEFAULT_OPERATIONS = Path("output/tex_gap_segmentation_control_correlation_probe/operations.csv")
GREEDY_MIN_TARGET_ROWS = 3
...
def control_bytes(row: dict[str, str]) -> list[str]:
    ...
    return [window[index : index + 2] for index in range(0, len(window), 2) if len(window[index : index + 2]) == 2]
...
def source_features(row: dict[str, str]) -> list[Feature]:
    features: list[Feature] = [
        ...
        ("len_mod", f"{row.get('length', '')}|{row.get('expected_mod64', '')}"),
...
def candidate_keys(row: dict[str, str]) -> list[CandidateKey]:
    ...
    len64 = ("len", "64")
    if len64 in features:
        for feature in features:
            if feature == len64:
                continue
            ordered = tuple(sorted((len64, feature)))
            keys.append(("len64_pair", ordered))
    return keys
...
def build_candidates(
    ...
    for (family, features), rows in grouped.items():
        ...
        selector_targets[row["selector"]] = {operation_key(item) for item in rows if operation_key(item) in target_keys}
...
def annotate_targets(
    ...
    for target in target_rows:
        ...
        false_free_count = sum(1 for row in false_free_candidates if key in selector_targets.get(row["selector"], set()))
        output_rows.append(
            {
...
def build_rows(
    ...
    len64_rows = [row for row in operation_rows if int_value(row, "length") == 64]
    ...
    summary = {
        ...
        "len64_operation_rows": str(len(len64_rows)),
        "len64_zero_rows": str(sum(1 for row in len64_rows if row.get("op_kind") == "zero")),
        "len64_false_rows": str(sum(1 for row in len64_rows if row.get("op_kind") != "zero")),
        ...
        "greedy_target_bytes": str(sum(int_value(operations_by_key[key], "length") for key in greedy_targets if key in operations_by_key)),
        "greedy_operation_rows": str(len(greedy_operations)),
        "greedy_zero_bytes": str(sum(int_value(row, "length") for row in greedy_operation_rows if row.get("op_kind") == "zero")),
        "greedy_false_bytes": str(sum(int_value(row, "length") for row in greedy_operation_rows if row.get("op_kind") != "zero")),
...
<header>
  <div class="wrap">
    ...
    <div class="sub">Scores source-side selectors for unresolved internal len64 zero targets.</div>
  </div>
</header>
<main class="wrap">
  <section class="stats">
...
<script>
const TEX_GAP_DECODER_LEN64_SELECTOR_PROBE = {data_json};
</script>
</body>
</html>
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Score source-side selectors for unresolved internal len64 zero targets."
    )
    parser.add_argument("--targets", type=Path, default=DEFAULT_TARGETS)
    parser.add_argument("--operations", type=Path, default=DEFAULT_OPERATIONS)
    parser.add_argument("-o", "--output", type=Path, default=DEFAULT_OUTPUT)
    parser.add_argument("--title", default="Lands of Lore II .tex Len64 Selector Probe")
    args = parser.parse_args()

    target_rows = read_csv(args.targets)
    operation_rows = read_csv(args.operations)
    summary, candidates, greedy, targets = build_rows(target_rows, operation_rows)
