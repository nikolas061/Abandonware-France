#!/usr/bin/env python3
"""Profile direct payload/source/spatial evidence for gradient-like .tex rows."""
...
DEFAULT_GRADIENT_TARGETS = Path("output/tex_gap_decoder_len64_promoted_tiny_nonzero_gap_gradient_probe/targets.csv")
DEFAULT_FIXTURES = Path("output/tex_gap_rule_fixtures/manifest.csv")
DEFAULT_REPLAY_FIXTURES = Path("output/tex_gap_decoder_len64_promoted_tiny_nonzero_fill_replay/fixtures.csv")
DEFAULT_OUTPUT = Path("output/tex_gradient_payload_profile")
...
def class_bytes(rows: list[dict[str, object]], classes: set[str]) -> int:
    return sum(int_value(row, "length") for row in rows if row.get("gradient_class") in classes)
...
def build(
    ...
    *,
    ...
    for row in gradient_rows:
        ...
        by_class_payloads[row.get("gradient_class", "")].append(target)
    ...
    for row, target, sources, row_issues in prepared:
        ...
        expected_all = sources.get("expected", b"")
        class_peer_exact, class_peer_ratio = best_peer_score(target, by_class_payloads[row.get("gradient_class", "")])
        ...
        if int(source_match["exact"]) == len(target) and int(source_match["known"]) == len(target):
            ...
            verdict = "gradient_payload_blocked"
        row_outputs.append(
            {
                "rank": row.get("rank", ""),
                ...
                "zero_delta_ratio": row.get("zero_delta_ratio", ""),
    ...
    _payload_groups, repeated_payload_rows, repeated_payload_bytes = repeated_row_stats(
        row_outputs, "payload_signature"
    )
    ...
    summary = {
        ...
        "gradient_classes": len({row.get("gradient_class", "") for row in row_outputs}),
        "small_delta_walk_bytes": class_bytes(row_outputs, {"small_delta_walk", "banded_small_delta_walk"}),
        "flat_run_walk_bytes": class_bytes(row_outputs, {"flat_run_walk"}),
        "banded_bytes": class_bytes(row_outputs, {"banded_small_delta_walk", "banded_mixed_walk"}),
...
<script type="application/json" id="gradient-payload-profile-data">{html.escape(data_json)}</script>
</body>
</html>
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Profile direct evidence for .tex gradient-like payloads.")
    parser.add_argument("--gradient-targets", type=Path, default=DEFAULT_GRADIENT_TARGETS)
    ...
    parser.add_argument("--title", default="Lands of Lore II .tex Gradient Payload Profile")
    args = parser.parse_args()

    summary, rows, groups, distances = build(
        read_csv(args.gradient_targets),
