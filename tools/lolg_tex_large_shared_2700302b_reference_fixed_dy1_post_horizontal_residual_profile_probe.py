#!/usr/bin/env python3
"""Profile residual pixels after guarded horizontal replay for shared 0x2700302b."""
...
DEFAULT_OUTPUT = Path("output/tex_large_shared_2700302b_reference_fixed_dy1_post_horizontal_residual_profile_probe")
DEFAULT_GUARDED_HORIZONTAL_SUMMARY = Path(
...
SUMMARY_FIELDNAMES = [
    ...
    "guarded_horizontal_remaining_nonzero_pixels",
    "guarded_horizontal_gap_pixels",
    "issue_rows",
    "post_horizontal_residual_verdict",
    "next_action",
]
...
def build_summary(
    ...
    if issue_rows:
        verdict = "shared_2700302b_reference_fixed_dy1_post_horizontal_residual_profile_issues"
        next_action = "fix shared 0x2700302b post-horizontal residual profile inputs"
    elif remaining == 0:
        verdict = "shared_2700302b_reference_fixed_dy1_post_horizontal_residual_clear"
        next_action = "promote shared 0x2700302b dy1 horizontal replay into decoder coverage"
    elif remaining <= 16:
        verdict = "shared_2700302b_reference_fixed_dy1_post_horizontal_terminal_residual"
        next_action = (
            "inspect terminal residual pixels after horizontal replay for shared 0x2700302b frontier "
            f"{guarded_summary.get('frontier_id', '')}; {remaining} pixels remain"
        )
    elif len(small) >= len(large):
        verdict = "shared_2700302b_reference_fixed_dy1_post_horizontal_small_delta_remaining"
        next_action = (
            "split post-horizontal residual small-delta misses for shared 0x2700302b frontier "
            f"{guarded_summary.get('frontier_id', '')}; {len(small)} small-delta and {len(large)} large-delta pixels remain"
        )
    else:
        verdict = "shared_2700302b_reference_fixed_dy1_post_horizontal_large_delta_remaining"
        next_action = (
            "derive post-horizontal large-delta producer for shared 0x2700302b frontier "
    ...
    return {
        ...
        "large_delta_aligned_pixels": guarded_summary.get("large_delta_aligned_pixels", "0"),
        "horizontal_selected_pixels": guarded_summary.get("horizontal_selected_pixels", "0"),
        ...
        "guarded_horizontal_remaining_nonzero_pixels": str(guarded_remaining),
        "guarded_horizontal_gap_pixels": str(guarded_gap),
        "issue_rows": str(issue_rows),
        "post_horizontal_residual_verdict": verdict,
...
def build_html(
    ...
    payload = {"summary": summary, "rows": rows, "deltas": deltas, "pixels": pixels}
...
def write_report(args: argparse.Namespace) -> tuple[dict[str, str], list[dict[str, str]]]:
    ...
    issues: list[str] = []
    guarded_summary = read_summary(args.guarded_horizontal_summary)
    if not guarded_summary:
        issues.append("missing_guarded_horizontal_summary")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Profile post-horizontal residuals for shared 0x2700302b.")
    parser.add_argument("-o", "--output", type=Path, default=DEFAULT_OUTPUT)
    parser.add_argument("--guarded-horizontal-summary", type=Path, default=DEFAULT_GUARDED_HORIZONTAL_SUMMARY)
    ...
    parser.add_argument(
        "--title",
        default="Lands of Lore II .tex Shared 0x2700302b Post-horizontal Residual Profile",
    )
    ...
    print(f"Verdict: {summary['post_horizontal_residual_verdict']}")
