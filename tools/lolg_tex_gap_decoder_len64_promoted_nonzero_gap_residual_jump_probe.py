#!/usr/bin/env python3
"""Probe sparse and long-island residual jump rows."""
...
from lolg_tex_gap_decoder_len64_promoted_nonzero_gap_mixed_jump_probe import (
    BAND_GROUP_FIELDNAMES,
    build_band_groups,
    islands_from_positions,
    jump_positions,
)
...
DEFAULT_OUTPUT = Path("output/tex_gap_decoder_len64_promoted_tiny_nonzero_gap_residual_jump_probe")
DEFAULT_TARGETS = Path("output/tex_gap_decoder_len64_promoted_tiny_nonzero_gap_jump_token_probe/targets.csv")
TARGET_CLASSES = {"sparse_jump_split", "long_island_split"}
...
def band_pair_key(pair: tuple[int, int]) -> str:
...
def build_target_rows(
    ...
    for target in target_rows:
        if target.get("jump_structure_class") not in TARGET_CLASSES:
            continue
        ...
        exact_pair_values = [f"{left:02x}>{right:02x}" for left, right, _delta in pairs]
        band_pairs = [tuple(sorted((left >> 4, right >> 4))) for left, right, _delta in pairs]
        dominant_band, dominant_count = Counter(band_pairs).most_common(1)[0] if band_pairs else ((), 0)
        zero_band_count = sum(1 for pair in band_pairs if 0 in pair)
        nibble_shape = ".".join(nibble_pairs)
        exact_shape = ".".join(exact_pair_values)
        ...
        exact_best, exact_windows = score_pools(exact_pair_values, pools, exact_label_sets)
        ...
        rows.append(
            {
                ...
                "jump_structure_class": target.get("jump_structure_class", ""),
                ...
                "dominant_band_pair": band_pair_key(dominant_band) if dominant_band else "",
                "dominant_band_count": str(dominant_count),
                "dominant_band_ratio": ratio(dominant_count, len(pairs)),
...
def build_class_groups(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    counters: dict[str, Counter[str]] = defaultdict(Counter)
    fixtures: dict[str, set[tuple[str, str, str]]] = defaultdict(set)
    samples: dict[str, dict[str, str]] = {}
    for row in rows:
        key = row.get("jump_structure_class", "")
        counters[key]["rows"] += 1
    ...
    output.sort(key=lambda row: (-int_value(row, "bytes"), row.get("jump_structure_class", "")))
    return output
...
def build_summary(rows: list[dict[str, str]], band_groups: list[dict[str, str]]) -> dict[str, str]:
    ...
    return {
        ...
        "sparse_rows": str(sum(1 for row in rows if row.get("jump_structure_class") == "sparse_jump_split")),
        "sparse_bytes": str(sum(int_value(row, "length") for row in rows if row.get("jump_structure_class") == "sparse_jump_split")),
        "long_island_rows": str(sum(1 for row in rows if row.get("jump_structure_class") == "long_island_split")),
        "long_island_bytes_total": str(sum(int_value(row, "length") for row in rows if row.get("jump_structure_class") == "long_island_split")),
...
def build_html(
    ...
    links = " ".join(
        ...
        for label, path in (
            ...
            ("by_jump_class.csv", output_dir / "by_jump_class.csv"),
...
<header>
  <div class="wrap">
    ...
    <div class="sub">Profiles sparse and long-island jump rows after dense, mixed, and repeated-nibble classes.</div>
  </div>
</header>
<main class="wrap">
  <section class="stats">
    <div class="stat"><div class="label">Residual bytes</div><div class="value">{summary['target_bytes']}</div></div>
    <div class="stat"><div class="label">Sparse rows</div><div class="value">{summary['sparse_rows']}</div></div>
  ...
  <section class="panel"><h2>Jump classes</h2>{render_table(class_groups, CLASS_GROUP_FIELDNAMES, 80)}</section>
...
<script>
const TEX_GAP_DECODER_LEN64_PROMOTED_NONZERO_GAP_RESIDUAL_JUMP_PROBE = {data_json};
</script>
</body>
</html>
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Probe residual sparse/long jump rows for .tex nonzero gaps.")
    ...
    parser.add_argument(
        "--title",
        default="Lands of Lore II .tex Tiny Nonzero Gap Residual Jump Probe",
    )
    args = parser.parse_args()

    rows = build_target_rows(read_csv(args.targets), read_csv(args.operations), read_csv(args.fixtures))
    class_groups = build_class_groups(rows)
    ...
    write_csv(args.output / "by_jump_class.csv", CLASS_GROUP_FIELDNAMES, class_groups)
    ...
    html_path.write_text(build_html(summary, rows, class_groups, band_groups, signal_groups, args.output, args.title))

    print(f"Residual-jump targets: {summary['target_rows']}")
    print(f"Residual-jump bytes: {summary['target_bytes']}")
    print(f"Sparse rows: {summary['sparse_rows']}")
