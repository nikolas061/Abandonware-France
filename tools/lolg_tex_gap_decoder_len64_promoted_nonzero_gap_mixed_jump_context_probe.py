#!/usr/bin/env python3
"""Check mixed-jump dominant bands for reusable payload context."""
...
DEFAULT_OUTPUT = Path("output/tex_gap_decoder_len64_promoted_tiny_nonzero_gap_mixed_jump_context_probe")
...
def normalize_rows(
        ...
        output.append(
            {
                ...
                "start": row.get("start", ""),
                "end": row.get("end", ""),
                "dominant_band_pair": row.get("dominant_band_pair", ""),
                "dominant_band_ratio": row.get("dominant_band_ratio", "0"),
                "zero_band_ratio": row.get("zero_band_ratio", "0"),
                "nibble_shape_key": row.get("nibble_shape_key", ""),
                "exact_shape_key": row.get("exact_shape_key", ""),
    ...
    output.sort(key=lambda item: (item.get("dominant_band_pair", ""), -int_value(item, "length"), item.get("pcx_name", "")))
    return output
        ...
        output.append(
            {
                ...
                "band_pair_keys": "|".join(sorted({row.get("dominant_band_pair", "") for row in group_rows})),
...
def build_band_rows(rows: list[dict[str, str]], payload_rows: list[dict[str, str]]) -> list[dict[str, str]]:
        ...
        output.append(
            {
                ...
                "dominant_band_rows": str(len(dominant_band)),
                "zero_band_rows": str(len(zero_band)),
    ...
    output.sort(key=lambda row: (-int_value(row, "bytes"), -int_value(row, "rows"), row.get("band_pair_key", "")))
    return output
...
def build_summary(
    ...
    dominant_band = [row for row in rows if float_value(row, "dominant_band_ratio") >= 0.50]
    zero_band = [row for row in rows if float_value(row, "zero_band_ratio") >= 0.50]
    ...
    return {
        ...
        "dominant_band_rows": str(len(dominant_band)),
        "dominant_band_bytes": str(sum(int_value(row, "length") for row in dominant_band)),
        "zero_band_rows": str(len(zero_band)),
        "zero_band_bytes": str(sum(int_value(row, "length") for row in zero_band)),
        "source_ge50_rows": str(len(source_ge50)),
...
def build_html(
    ...
    payload = {"summary": summary, "targets": rows, "bandRows": band_rows, "payloadRows": payload_rows}
...
<header>
  <div class="wrap">
    ...
    <div class="sub">Checks mixed-jump dominant bands for exact payload reuse.</div>
  </div>
</header>
<main class="wrap">
  <section class="stats">
...
<script>
const TEX_GAP_DECODER_LEN64_PROMOTED_NONZERO_GAP_MIXED_JUMP_CONTEXT_PROBE = {data_json};
</script>
</body>
</html>
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Probe mixed-jump band context and payload reuse.")
    parser.add_argument("--targets", type=Path, default=DEFAULT_TARGETS)
    parser.add_argument("--fixtures", type=Path, default=DEFAULT_FIXTURES)
    parser.add_argument("-o", "--output", type=Path, default=DEFAULT_OUTPUT)
    parser.add_argument(
        "--title",
        default="Lands of Lore II .tex Tiny Nonzero Gap Mixed Jump Context Probe",
    )
    args = parser.parse_args()

    rows = normalize_rows(read_csv(args.targets), read_fixture_csv(args.fixtures))
    payload_rows = build_payload_rows(rows)
    band_rows = build_band_rows(rows, payload_rows)
    summary = build_summary(rows, band_rows, payload_rows)
    ...
    html_path.write_text(build_html(summary, rows, band_rows, payload_rows, args.output, args.title))

    print(f"Mixed-jump context rows: {summary['target_rows']}")
