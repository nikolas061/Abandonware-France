#!/usr/bin/env python3
"""Probe backward-copy distances for jump-token nonzero gap rows."""
...
DEFAULT_OUTPUT = Path("output/tex_gap_decoder_len64_promoted_tiny_nonzero_gap_jump_token_backref_probe")
...
def rule_conditions(row: dict[str, str]) -> list[tuple[str, str]]:
    ...
    return [
        ...
        ("distance_jump_structure_class", f"jump_structure_class={row.get('jump_structure_class', '')}"),
        ("distance_island_length_shape", f"island_length_shape={row.get('island_length_shape_key', '')}"),
        ("distance_jump_signed_shape", f"jump_signed_shape={row.get('jump_signed_shape_key', '')}"),
        ("distance_jump_nibble_pair", f"jump_nibble_pair={row.get('jump_nibble_pair_key', '')}"),
        (
            "distance_jump_shape_pair",
            f"jump_signed_shape={row.get('jump_signed_shape_key', '')}|"
            f"jump_nibble_pair={row.get('jump_nibble_pair_key', '')}",
        ),
    ]
...
def target_base_row(target: dict[str, str], expected: bytes) -> dict[str, str]:
    return {
        ...
        "island_length_shape_key": target.get("island_length_shape_key", ""),
        "jump_signed_shape_key": target.get("jump_signed_shape_key", ""),
        "jump_nibble_pair_key": target.get("jump_nibble_pair_key", ""),
        "best_distance": "",
        "best_source_start": "",
        "best_source_end": "",
...
<header>
  <div class="wrap">
    ...
    <div class="sub">Scores backward copy distances for jump-token rows and separates exact copies by known source coverage.</div>
  </div>
</header>
<main class="wrap">
  <section class="stats">
    <div class="stat"><div class="label">Jump-token bytes</div><div class="value">{summary['target_bytes']}</div></div>
...
<script>
const TEX_GAP_DECODER_LEN64_PROMOTED_NONZERO_GAP_JUMP_TOKEN_BACKREF_PROBE = {data_json};
</script>
</body>
</html>
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Probe .tex jump-token backward copy distances.")
    ...
    parser.add_argument(
        "--title",
        default="Lands of Lore II .tex Tiny Nonzero Gap Jump Token Backref Probe",
    )
