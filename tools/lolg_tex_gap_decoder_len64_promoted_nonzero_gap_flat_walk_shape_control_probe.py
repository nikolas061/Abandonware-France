#!/usr/bin/env python3
"""Correlate flat-walk run and transition shapes with fixed control selectors."""
...
DEFAULT_OUTPUT = Path("output/tex_gap_decoder_len64_promoted_tiny_nonzero_gap_flat_walk_shape_control_probe")
DEFAULT_SHAPES = Path("output/tex_gap_decoder_len64_promoted_tiny_nonzero_gap_flat_walk_probe/targets.csv")
...
def add_selector(selectors: list[tuple[str, str]], family: str, key: str) -> None:
...
def build_selector_keys(target: dict[str, str], operation: dict[str, str]) -> list[tuple[str, str]]:
    ...
    value_runs = target.get("value_run_count", "")
    transitions = target.get("transition_count", "")
    max_run = target.get("max_value_run", "")
    add_selector(selectors, "length", f"length={length}")
    add_selector(selectors, "length_bucket", f"length_bucket={length_bucket(length)}")
    add_selector(selectors, "value_run_count", f"value_run_count={value_runs}")
    add_selector(selectors, "transition_count", f"transition_count={transitions}")
    add_selector(selectors, "max_value_run", f"max_value_run={max_run}")
    ...
    add_selector(
        selectors,
        "run_count_control_ref_mod64",
        f"value_run_count={value_runs}|control_ref_mod64={target.get('control_ref_mod64', '')}",
    )
...
def build_rows(
    ...
    for target in shape_rows:
        ...
        for shape_kind, shape_field in (
            ("run_length_shape", "run_length_shape_key"),
            ("transition_shape", "transition_shape_key"),
            ("run_value_shape", "run_value_shape_key"),
        ):
...
<header>
  <div class="wrap">
    ...
    <div class="sub">Scores whether fixed metadata or control-byte selectors isolate flat-walk run, transition, and run-value shapes.</div>
  </div>
</header>
<main class="wrap">
  <section class="stats">
  ...
  <section class="panel"><h2>Selector families</h2>{render_table(families, FAMILY_FIELDNAMES, 120)}</section>
  <section class="panel"><h2>Selector candidates</h2>{render_table(selectors, SELECTOR_FIELDNAMES, 420)}</section>
</main>
<script>
const TEX_GAP_DECODER_LEN64_PROMOTED_NONZERO_GAP_FLAT_WALK_SHAPE_CONTROL_PROBE = {data_json};
</script>
</body>
</html>
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Correlate .tex flat-walk shapes with controls.")
    parser.add_argument("--shapes", type=Path, default=DEFAULT_SHAPES)
    parser.add_argument("--operations", type=Path, default=DEFAULT_OPERATIONS)
    parser.add_argument("-o", "--output", type=Path, default=DEFAULT_OUTPUT)
    parser.add_argument(
        "--title",
        default="Lands of Lore II .tex Tiny Nonzero Gap Flat Walk Shape Control Probe",
    )
    ...
    print(f"Flat-walk shape targets: {summary['target_rows']}")
