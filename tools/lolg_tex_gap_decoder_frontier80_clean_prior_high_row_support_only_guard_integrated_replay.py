#!/usr/bin/env python3
"""Integrate the support-only guard into the high-row selector replay."""
...
import lolg_tex_gap_decoder_frontier80_clean_prior_high_row_support_only_guard_promoted_replay as guard_replay
from lolg_tex_gap_opcode_probe import int_value, relative_href, write_csv


DEFAULT_OUTPUT = Path("output/tex_gap_decoder_frontier80_clean_prior_high_row_support_only_guard_integrated_replay")
DEFAULT_OBSERVATIONS = guard_replay.DEFAULT_OBSERVATIONS
DEFAULT_OPERATIONS = Path(
    "output/tex_gap_decoder_frontier80_clean_prior_high_row_support_only_guard_promoted_replay/operations.csv"
)
...
def build_rows(
    *,
    ...
    replay_summary, replay_operations, decisions, replay_issues = guard_replay.build_replay(
        observation_rows,
        operation_rows,
    )
    operations_by_key = {row_key(row): row for row in replay_operations if row.get("resolved_outlier") == "1"}
    ...
    clean = (
        replay_summary.get("review_verdict") == "frontier80_prior_high_row_support_only_guard_promoted_replay_ready"
    ...
    summary = {
        ...
        "review_verdict": (
            "frontier80_prior_high_row_support_only_guard_integrated_replay_ready"
            if clean
            else "frontier80_prior_high_row_support_only_guard_integrated_replay_weak"
        ),
        "next_probe": (
            "derive exact residual correction for support-only high-row selector"
            if clean
            else "review support-only high-row selector integration issues"
        ),
    }
...
<header>
  <div class="wrap">
    ...
    <div class="sub">Integrates support-only guard replay rows as the high-row selector boundary.</div>
  </div>
</header>
<main class="wrap">
  <section class="stats">
...
<script type="application/json" id="support-only-guard-integrated-replay-data">{data_json}</script>
</body>
</html>
"""
...
def write_report(
    ...
    *,
    ...
    summary, selector_rows, decision_rows, issues = build_rows(
        observation_rows=read_csv(observations_path),
        operation_rows=read_csv(operations_path),
        source_rows=read_csv(sources_path),
        base_fixture_rows=read_csv(base_fixtures_path),
    )
    write_csv(output_dir / "summary.csv", SUMMARY_FIELDNAMES, [summary])
    write_csv(output_dir / "selector_rows.csv", SELECTOR_FIELDNAMES, selector_rows)
    write_csv(output_dir / "guard_decisions.csv", guard_replay.DECISION_FIELDNAMES, decision_rows)
    (output_dir / "issues.txt").write_text("\n".join(issues) + ("\n" if issues else ""))
    (output_dir / "index.html").write_text(build_html(summary, selector_rows, decision_rows, output_dir, title))
    return summary


def main() -> None:
    parser = argparse.ArgumentParser(description="Integrate support-only high-row guard replay rows.")
    ...
    parser.add_argument(
        "--title",
        default="Lands of Lore II .tex Frontier80 Prior High Row Support-Only Guard Integrated Replay",
    )
    ...
    print(f"Le2 gain: {summary['le2_gain_bytes']}")
