#!/usr/bin/env python3
"""Review terminal-source byte guards for residual high-safe source chains."""
...
DEFAULT_OUTPUT = Path("output/tex_gradient_sequence_high_safe_low_exception_terminal_source_byte_guard")
...
BYTE_BASES = [
    "root_target_byte",
    "source_slot_target_byte",
    "terminal_target_byte",
...
LOW_FIELDS = [
    ...
    "root_prev_low1",
    "root_prev_low2",
    "source_slot_prev_low1",
    "source_slot_prev_low2",
    "terminal_prev_low1",
    "terminal_prev_low2",
    "root_rel_mod4",
    "source_slot_rel_mod4",
    "terminal_rel_mod4",
    "root_prefix_low",
    "root_fragment_low",
    "source_slot_prefix_low",
    "source_slot_fragment_low",
    "terminal_prefix_low",
...
FEATURES = [
    ...
    "source_slot_prev_low2",
    "source_slot_rel_mod4",
    "source_slot_rel_mod8",
    "source_slot_seq_index",
    "source_slot_prefix_low",
    "source_slot_fragment_low",
...
def follow_terminal(
    source_slot_rank: str,
    slots_by_rank: dict[str, dict[str, str]],
    edges: dict[str, str],
) -> tuple[dict[str, str], list[str]]:
    path: list[str] = []
    seen: set[str] = set()
    current = source_slot_rank
    while current and current not in seen:
        ...
        if not next_rank:
            break
        current = next_rank
    return slots_by_rank.get(current, {}), path


def nibble(value: str) -> int | None:
    value = str(value).lower()
    if value and value[-1:] in "0123456789abcdef":
        return int(value[-1], 16)
    return None


def byte_value(value: str) -> int | None:
    value = str(value).lower()
    if len(value) != 2:
        return None
    try:
...
def enrich_row(
    ...
    enriched: dict[str, str] = {
        "_slot_rank": row.get("rank", ""),
        "_expected_byte": row.get("source_expected_byte", "").lower(),
        "_is_unknown": "1" if target_scope(row) else "0",
        "_is_known": "1" if known_scope(row) else "0",
        "edge_key": source_dependency_edge(row),
        "path_frontiers": "->".join(slots_by_rank.get(rank, {}).get("frontier_id", "") for rank in path),
        "terminal_source_byte": terminal.get("source_expected_byte", "").lower(),
    }
    enriched["terminal_source_high"] = enriched["terminal_source_byte"][:1]
    enriched["terminal_source_low"] = enriched["terminal_source_byte"][-1:]
    for prefix, source in (
        ...
        for field in (
            ...
            enriched[f"{prefix}_{field}"] = source.get(field, "")
    return enriched


def terminal_source_rows(slot_rows: list[dict[str, str]]) -> list[dict[str, str]]:
    slots_by_rank = {row["rank"]: row for row in slot_rows}
    edges = unknown_highsafe_edges(slot_rows)
    rows: list[dict[str, str]] = []
    for row in slot_rows:
        if not source_scope(row):
            continue
        source_slot = slots_by_rank.get(row.get("source_slot_rank", ""), {})
        terminal, path = follow_terminal(row.get("source_slot_rank", ""), slots_by_rank, edges)
        if terminal.get("source_availability") != "known_source":
            continue
        rows.append(enrich_row(row, source_slot, terminal, path, slots_by_rank))
    return rows


def formula_predictions(rows: list[dict[str, str]]) -> list[tuple[str, dict[str, str]]]:
    known_values = sorted({row["_expected_byte"] for row in rows if row.get("_is_known") == "1"})
    predictions: list[tuple[str, dict[str, str]]] = []
    for value in known_values:
        predictions.append((value, {row["_slot_rank"]: value for row in rows}))
    for base in BYTE_BASES:
        for delta in BYTE_DELTAS:
            name = base if delta == 0 else f"{base}{delta:+d}"
            formula: dict[str, str] = {}
            for row in rows:
                value = byte_value(row.get(base, ""))
                formula[row["_slot_rank"]] = "" if value is None else byte_text(value + delta)
            predictions.append((name, formula))
    for field in LOW_FIELDS:
        for delta in FORMULA_DELTAS:
            name = f"6+{field}" if delta == 0 else f"6+{field}{delta:+d}"
            formula = {}
            for row in rows:
                value = nibble(row.get(field, ""))
                formula[row["_slot_rank"]] = "" if value is None else f"6{(value + delta) & 0x0F:x}"
            predictions.append((name, formula))
    return predictions
...
def build_guard_rows(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    for formula_name, predictions in formula_predictions(rows):
        if not any(
            predictions[row["_slot_rank"]] == row["_expected_byte"] and row.get("_is_unknown") == "1"
            for row in rows
        ):
            continue
        if not any(
            predictions[row["_slot_rank"]] == row["_expected_byte"] and row.get("_is_known") == "1"
            for row in rows
        ):
            continue
        for fields in feature_sets():
            ...
            for row in rows:
                prediction = predictions[row["_slot_rank"]]
                if not prediction:
                    continue
                exact = prediction == row["_expected_byte"]
                key = guard_key(row, fields)
                if row.get("_is_unknown") == "1":
                    groups[key]["unknown_exact_rows" if exact else "unknown_false_rows"] += 1
                elif row.get("_is_known") == "1":
    ...
    for index, row in enumerate(guards, start=1):
        row["rank"] = str(index)
    return guards


def select_guard(guards: list[dict[str, str]]) -> dict[str, str]:
    for row in guards:
...
def review_verdict(guard: dict[str, str]) -> str:
    if not guard:
        return "missing_terminal_source_byte_guard"
    if int_value(guard, "known_exact_rows") <= 0:
        return "terminal_source_byte_guard_missing_known_support"
    if int_value(guard, "known_false_rows") > 0:
        return "terminal_source_byte_guard_known_false"
    if int_value(guard, "unknown_false_rows") > 0:
        return "terminal_source_byte_guard_unknown_false"
    if int_value(guard, "unknown_exact_rows") <= 0:
        return "terminal_source_byte_guard_no_targets"
    return "terminal_source_byte_guard_promotion_ready"


def next_probe_for(verdict: str) -> str:
    if verdict == "terminal_source_byte_guard_promotion_ready":
        return "promote guarded terminal-source bytes for high-safe chains"
    return "derive broader terminal-source byte transform for known-terminal chains"


def predicted_byte(row: dict[str, str], guard: dict[str, str], predictions: dict[str, dict[str, str]]) -> str:
    return predictions.get(guard.get("formula", ""), {}).get(row.get("rank", ""), "")


def matches_guard(row: dict[str, str], guard: dict[str, str], enriched: dict[str, str]) -> bool:
    fields = tuple(guard.get("guard_family", "").split("+"))
    return guard_key(enriched, fields) == guard.get("guard_key", "")


def build(
    slot_rows: list[dict[str, str]],
) -> tuple[dict[str, str], list[dict[str, str]], list[dict[str, str]]]:
    enriched_rows = terminal_source_rows(slot_rows)
    guards = build_guard_rows(enriched_rows)
    guard = select_guard(guards)
    verdict = review_verdict(guard)
    formula_map = {name: values for name, values in formula_predictions(enriched_rows)}
    enriched_by_rank = {row["_slot_rank"]: row for row in enriched_rows}
    target_rows: list[dict[str, str]] = []
    seen_target_keys: set[tuple[str, str, str, str]] = set()
    if guard:
        for row in slot_rows:
            enriched = enriched_by_rank.get(row.get("rank", ""))
            if not enriched or not target_scope(row) or not matches_guard(row, guard, enriched):
                continue
            prediction = formula_map.get(guard.get("formula", ""), {}).get(row.get("rank", ""), "")
            if not prediction:
                continue
            issues: list[str] = []
            ...
            target_key = (row.get("archive", ""), row.get("pcx_name", ""), row.get("frontier_id", ""), source_offset)
            ...
            target_rows.append(
                {
                    ...
                    "promotion_ready": "1" if verdict == "terminal_source_byte_guard_promotion_ready" and not issues else "0",
                    "issues": ";".join(issues),
                }
            )
    promotion_ready = (
        len(target_rows)
        if verdict == "terminal_source_byte_guard_promotion_ready" and not any(row.get("issues") for row in target_rows)
        else 0
    )
    summary = {
        "scope": "total",
        "candidate_mode": "high_safe_low_exception_terminal_source_byte_guard_review",
        ...
        "terminal_source_rows": str(len(enriched_rows)),
    ...
    return summary, guards, target_rows
...
<script type="application/json" id="terminal-source-byte-guard-review-data">{html.escape(data_json)}</script>
</body>
</html>
"""
...
def main() -> None:
    ...
    parser.add_argument("--title", default="Lands of Lore II .tex Terminal Source Byte Guard Review")
    ...
    print(
        "Terminal-source byte guard review: "
