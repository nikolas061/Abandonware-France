#!/usr/bin/env python3
"""Score fixed small-palette selector rules for nonzero gap palettes."""
...
DEFAULT_OUTPUT = Path("output/tex_gap_decoder_len64_promoted_tiny_nonzero_gap_palette_selector_probe")
...
def repeat_palette(palette: bytes, length: int) -> bytes:
    if not palette:
        return b""
    repeats = (length + len(palette) - 1) // len(palette)
    return (palette * repeats)[:length]
...
def build_targets(
    ...
    target_expected: list[bytes] = []

    for pattern in pattern_rows:
        if pattern.get("pattern_class") not in SMALL_PALETTE_CLASSES:
            continue
        ...
        palette_size = len(unique)
        if palette_size not in (2, 3, 4):
            issues.append(f"unexpected_palette_size:{palette_size}")
            continue
    ...
    return target_rows, target_pools, target_expected, fixture_issues
        ...
        for pool_name, pool in pools.items():
            for transform in FIXED_TRANSFORMS:
                ...
                for offset in range(0, len(transformed) - palette_size + 1):
                    ...
                    has_palette_values = len(selected_set) == palette_size and expected_unique.issubset(selected_set)
...
def build_rule_rows(
    ...
    for key, counter in counters.items():
        family, pool, transform, palette_size, offset, condition = key
...
def rule_label(row: dict[str, str]) -> str:
    return (
        ...
        f"palette={row.get('palette_size')}|offset={row.get('source_offset')}|{row.get('condition')}"
    )
...
def build_family_rows(rule_rows: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    for family, rules in by_family.items():
        best = max(
            rules,
            key=lambda row: (int_value(row, "correct_bytes"), int_value(row, "exact_bytes"), -int_value(row, "false_bytes")),
        )
        rows.append(
            {
                "rule_family": family,
                ...
                "exact_false_free_rules": str(sum(1 for row in rules if int_value(row, "exact_false_bytes") == 0)),
...
def build_summary(
    ...
    palette2 = [row for row in target_rows if row.get("pattern_class") == "small_palette_2"]
    palette4 = [row for row in target_rows if row.get("pattern_class") == "small_palette_4"]
    ...
    return {
        ...
        "target_bytes": str(sum_bytes(target_rows)),
...
<header>
  <div class="wrap">
    ...
    <div class="sub">Scores non-oracle fixed offset selectors for small palette nonzero gap targets.</div>
  </div>
</header>
<main class="wrap">
  <section class="stats">
    <div class="stat"><div class="label">Palette bytes</div><div class="value">{summary['target_bytes']}</div></div>
    <div class="stat"><div class="label">Palette-2 bytes</div><div class="value">{summary['small_palette_2_bytes']}</div></div>
    <div class="stat"><div class="label">Palette-4 bytes</div><div class="value">{summary['small_palette_4_bytes']}</div></div>
    ...
    <div class="stat"><div class="label">Best exact bytes</div><div class="value">{summary['best_any_exact_bytes']}</div></div>
  </section>
...
<script>
const TEX_GAP_DECODER_LEN64_PROMOTED_NONZERO_GAP_PALETTE_SELECTOR_PROBE = {data_json};
</script>
</body>
</html>
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Score fixed small-palette selectors for .tex nonzero gaps.")
    ...
    parser.add_argument(
        "--title",
        default="Lands of Lore II .tex Tiny Nonzero Gap Palette Selector Probe",
    )
