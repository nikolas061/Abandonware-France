#!/usr/bin/env python3
"""Profile residual pixels after guarded restart-low2 replay for shared 0x2700302b."""
...
DEFAULT_OUTPUT = Path("output/tex_large_shared_2700302b_reference_fixed_dy1_post_restart_low2_residual_profile_probe")
...
SUMMARY_FIELDNAMES = [
    ...
    "guarded_restart_remaining_nonzero_pixels",
    "guarded_restart_gap_pixels",
    "issue_rows",
    "post_restart_low2_residual_verdict",
    "next_action",
]
...
def build_summary(
    ...
    if issue_rows:
        verdict = "shared_2700302b_reference_fixed_dy1_post_restart_low2_residual_profile_issues"
        next_action = "fix shared 0x2700302b post-restart-low2 residual profile inputs"
    elif len(small) >= len(large):
        verdict = "shared_2700302b_reference_fixed_dy1_post_restart_low2_small_delta_remaining"
        next_action = (
            "split post-restart-low2 residual small-delta misses for shared 0x2700302b frontier "
            f"{guarded_summary.get('frontier_id', '')}; {len(small)} small-delta and {len(large)} large-delta pixels remain"
        )
    else:
        verdict = "shared_2700302b_reference_fixed_dy1_post_restart_low2_large_delta_focus"
        next_action = (
            "derive large-delta/source-zero producer after restart low2 for shared 0x2700302b frontier "
    ...
    return {
        ...
        "guarded_restart_remaining_nonzero_pixels": str(guarded_remaining),
        "guarded_restart_gap_pixels": str(guarded_gap),
        "issue_rows": str(issue_rows),
        "post_restart_low2_residual_verdict": verdict,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Profile post-restart-low2 residuals for shared 0x2700302b.")
    ...
    parser.add_argument(
        "--title",
        default="Lands of Lore II .tex Shared 0x2700302b Post-restart-low2 Residual Profile",
    )
    ...
    print(f"Verdict: {summary['post_restart_low2_residual_verdict']}")
