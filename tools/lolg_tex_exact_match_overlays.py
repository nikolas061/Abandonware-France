#!/usr/bin/env python3
"""Render Full HD overlays for direct .tex/CDCACHE chunk matches."""
...
DEFAULT_OUTPUT = Path("output/tex_exact_match_overlays")
...
def grouped_matches(rows: list[dict[str, str]]) -> dict[tuple[str, str], list[dict[str, str]]]:
    output: dict[tuple[str, str], list[dict[str, str]]] = {}
    for row in rows:
        if row.get("issues") or not row.get("segment_offset"):
            continue
        key = (row.get("archive", ""), row.get("pcx_name", ""))
        if key[0] and key[1]:
...
def render_overlay(
    *,
    ...
    for row in match_rows:
        pixel_offset = int(row.get("pixel_offset") or 0)
        chunk_size = int(row.get("chunk_size") or 0)
        for delta in range(chunk_size):
            offset = pixel_offset + delta
...
def build_rows(matches: Path, comparisons: Path, output_dir: Path) -> list[dict[str, str]]:
    match_rows = read_rows(matches)
    comparison_rows = read_rows(comparisons)
    comparisons_by_key = comparison_lookup(comparison_rows)
    matches_by_key = grouped_matches(match_rows)
    ...
    for key, group in sorted(matches_by_key.items()):
        ...
        if not native_path.exists():
            ...
            with Image.open(native_path) as source:
                overlay, covered = render_overlay(image=source, match_rows=group)
            width, height = overlay.size
            stem = safe_stem(Path(archive).stem, pcx_name)
            native_output = native_dir / f"{stem}_match_overlay.png"
            fullhd_output = fullhd_dir / f"{stem}_match_overlay_fullhd.png"
...
def summary_row(rows: list[dict[str, str]]) -> dict[str, str]:
    ...
    return {
        ...
        "covered_pixels": str(sum(int(row.get("covered_pixels") or 0) for row in rows)),
...
<article class="card">
  ...
  <div class="card-body">
    ...
    <div class="muted">{html.escape(row.get('archive_tag', ''))} - {html.escape(row.get('match_rows', ''))} chunks - {html.escape(row.get('covered_pixels', ''))} pixels</div>
...
<main class="wrap">
  <section class="stats">
    ...
    <div class="stat"><div class="label">Full HD</div><div class="value">{html.escape(summary['fullhd_overlays'])}</div></div>
...
<script>
const TEX_EXACT_MATCH_OVERLAYS = {data_json};
</script>
</body>
</html>
"""
...
def write_report(output_dir: Path, matches: Path, comparisons: Path, title: str) -> tuple[dict[str, str], list[dict[str, str]]]:
    output_dir.mkdir(parents=True, exist_ok=True)
    rows = build_rows(matches, comparisons, output_dir)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Render Full HD overlays for direct .tex chunk matches.")
    ...
    parser.add_argument("--title", default="Lands of Lore II .tex Exact Match Overlays")
    args = parser.parse_args()

    summary, _rows = write_report(args.output, args.matches, args.comparisons, args.title)
