#!/usr/bin/env python3
"""Correlate residual sparse/long jump rows with control/source byte signals."""
...
DEFAULT_OUTPUT = Path("output/tex_gap_decoder_len64_promoted_tiny_nonzero_gap_residual_control_probe")
DEFAULT_TARGETS = Path("output/tex_gap_decoder_len64_promoted_tiny_nonzero_gap_residual_jump_probe/targets.csv")
...
def build_summary(rows: list[dict[str, str]]) -> dict[str, str]:
    ...
    return {
        ...
        "long_island_bytes_total": str(
            sum(int_value(row, "length") for row in rows if row.get("jump_structure_class") == "long_island_split")
        ),
...
<header>
  <div class="wrap">
    ...
    <div class="sub">Scores sparse and long-island residual jump rows against control/source signal families.</div>
  </div>
</header>
<main class="wrap">
  <section class="stats">
    ...
    <div class="stat"><div class="label">Long phase rows >=75%</div><div class="value ok">{summary['phase_ge75_long_rows']}</div></div>
...
<script>
const TEX_GAP_DECODER_LEN64_PROMOTED_NONZERO_GAP_RESIDUAL_CONTROL_PROBE = {data_json};
</script>
</body>
</html>
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Probe residual sparse/long jump rows against control signals.")
    ...
    parser.add_argument(
        "--title",
        default="Lands of Lore II .tex Tiny Nonzero Gap Residual Control Probe",
    )
    ...
    html_path.write_text(build_html(summary, rows, class_groups, signal_groups, args.output, args.title))

    print(f"Residual-control targets: {summary['target_rows']}")
    print(f"Residual-control bytes: {summary['target_bytes']}")
    print(f"Phase rows >=75%: {summary['phase_ge75_rows']}")
    print(f"Long phase rows >=75%: {summary['phase_ge75_long_rows']}")
