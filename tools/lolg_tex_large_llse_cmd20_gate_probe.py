#!/usr/bin/env python3
"""Test gated LLSE cmd20 semantics against the broad high-arg2 candidate."""
...
DEFAULT_OUTPUT = Path("output/tex_large_llse_cmd20_gate_probe")
...
DEFAULT_CMD20_TUPLES = Path("output/tex_large_llse_command_record_probe/cmd20.csv")
...
class GateVariant:
    ...
    def variant_id(self) -> str:
        return (
            f"gate-{self.gate_policy}_non20-{self.noncmd20_policy}_"
            f"smallxy-{self.smallxy_policy}_z{self.zero_policy}_"
            f"dy{self.dy_policy}_thr{self.threshold:02x}_x{self.x_policy}_skip{self.skip}"
        )
...
def parse_tuple(value: str) -> tuple[int, int, int] | None:
...
def load_tuple_whitelists(path: Path, top8: int, top32: int) -> dict[str, set[tuple[int, int, int]]]:
    rows = read_csv(path)
    parsed = [parse_tuple(row.get("arg_tuple", "")) for row in rows]
    tuples = [value for value in parsed if value is not None]
    return {
        "top8": set(tuples[:top8]),
        "top32": set(tuples[:top32]),
    }
...
def is_zero_tuple(arg1: int, arg2: int, arg3: int) -> bool:
    return (arg1, arg2, arg3) == (0, 0, 0)


def is_higharg2_tuple(arg2: int, threshold: int) -> bool:
    return arg2 >= threshold


def is_smallxy_tuple(arg1: int, arg2: int, arg3: int) -> bool:
    return arg3 == 0 and 0 < arg1 < 0x40 and 0 < arg2 <= 0x20 and arg2 % 4 == 0
...
def command_gate(
    ...
    arg2: int,
    arg3: int,
    tuple_whitelists: dict[str, set[tuple[int, int, int]]],
) -> bool:
    current = (arg1, arg2, arg3)
    if variant.gate_policy == "all":
        return True
    if variant.gate_policy == "zero_or_high":
        return is_zero_tuple(arg1, arg2, arg3) or is_higharg2_tuple(arg2, variant.threshold)
    if variant.gate_policy == "high_only":
        return is_higharg2_tuple(arg2, variant.threshold)
    if variant.gate_policy == "zero_only":
        return is_zero_tuple(arg1, arg2, arg3)
    if variant.gate_policy == "smallxy_only":
        return is_smallxy_tuple(arg1, arg2, arg3)
    if variant.gate_policy == "zero_high_smallxy":
        return (
            is_zero_tuple(arg1, arg2, arg3)
            or is_higharg2_tuple(arg2, variant.threshold)
            or is_smallxy_tuple(arg1, arg2, arg3)
        )
    if variant.gate_policy == "arg3zero_control":
        return arg3 == 0 and (
            is_zero_tuple(arg1, arg2, arg3)
            or is_higharg2_tuple(arg2, variant.threshold)
            or is_smallxy_tuple(arg1, arg2, arg3)
        )
    if variant.gate_policy in tuple_whitelists:
        return current in tuple_whitelists[variant.gate_policy]
    return False


def build_variants(summary: dict[str, str]) -> list[GateVariant]:
    ...
    for gate_policy in (
        ...
        noncmd20_policies = ("ignore",) if gate_policy == "all" else ("ignore", "advance", "emit")
        smallxy_policies = (
            ("none", "setx", "ydiv4")
            if gate_policy in {"smallxy_only", "zero_high_smallxy", "arg3zero_control", "top8", "top32"}
            else ("none",)
        )
        for noncmd20_policy in noncmd20_policies:
            for smallxy_policy in smallxy_policies:
                variants.append(
                    GateVariant(
                        gate_policy=gate_policy,
                        noncmd20_policy=noncmd20_policy,
                        smallxy_policy=smallxy_policy,
...
def apply_noncmd20(
    ...
    if policy == "emit":
        put_pixel(pixels, width, height, x, y, 0x20)
        stats["noncmd20_emit"] += 1
        return advance(x, y, width, 1)
    if policy == "advance":
        stats["noncmd20_advance"] += 1
        return advance(x, y, width, 1)
    return x, y
...
def decode_gated(
    ...
    stats = {
        ...
        "cmd20_applied": 0,
        "cmd20_ignored": 0,
        "cmd20_zero_applied": 0,
        "cmd20_higharg2_applied": 0,
        "cmd20_smallxy_applied": 0,
    ...
    while pos < len(payload) and y < height:
        ...
        if byte == 0x20:
            ...
            if not command_gate(variant, arg1, arg2, arg3, tuple_whitelists):
                stats["cmd20_ignored"] += 1
                x, y = apply_noncmd20(pixels, width, height, x, y, variant.noncmd20_policy, stats)
                continue
            stats["cmd20_applied"] += 1
            if is_zero_tuple(arg1, arg2, arg3):
                stats["cmd20_zero_applied"] += 1
                ...
                if variant.zero_policy == "skip":
                    ...
                    continue
                continue
            if is_higharg2_tuple(arg2, variant.threshold):
                dy = signed_byte(arg2)
                ...
                if 0 <= next_y < height:
                    ...
                    continue
            if is_smallxy_tuple(arg1, arg2, arg3) and variant.smallxy_policy != "none":
                if variant.smallxy_policy == "setx":
                    x = arg1 % max(1, width)
                elif variant.smallxy_policy == "ydiv4":
                    x = arg1 % max(1, width)
                    y = min(height, y + max(1, arg2 // 4))
                pos = min(len(payload), pos + variant.skip)
                stats["cmd20_smallxy_applied"] += 1
                continue
            pos = min(len(payload), pos + variant.skip)
...
def evaluate_variant(
    ...
    stats: dict[str, int] = {}
    if not issues:
        pixels, stats = decode_gated(payload, width, height, variant, tuple_whitelists, low, high)
    ...
    return (
        {
            ...
            "gate_policy": variant.gate_policy,
            "noncmd20_policy": variant.noncmd20_policy,
            "smallxy_policy": variant.smallxy_policy,
            ...
            "tuple_whitelist": str(len(tuple_whitelists.get(variant.gate_policy, set()))),
            "offset": str(offset),
            "width": str(width),
            "height": str(height),
            ...
            "cmd20_applied": str(stats.get("cmd20_applied", "")),
            "cmd20_ignored": str(stats.get("cmd20_ignored", "")),
            "cmd20_zero_applied": str(stats.get("cmd20_zero_applied", "")),
            "cmd20_higharg2_applied": str(stats.get("cmd20_higharg2_applied", "")),
            "cmd20_smallxy_applied": str(stats.get("cmd20_smallxy_applied", "")),
            "cmd20_generic_skipped": str(stats.get("cmd20_generic_skipped", "")),
            "noncmd20_emit": str(stats.get("noncmd20_emit", "")),
            "noncmd20_advance": str(stats.get("noncmd20_advance", "")),
...
def write_previews(
    ...
    for row in rows[:limit]:
        ...
        sheet_entries.append((f"{row['rank']} {row['score']} {row['gate_policy']}", image))
...
def summary_row(rows: list[dict[str, str]], higharg2_summary: dict[str, str], preview_limit: int) -> dict[str, str]:
    ...
    if issue_rows:
        verdict = "llse_cmd20_gate_probe_issues"
        next_action = "fix LLSE cmd20 gate probe inputs"
    elif best and best_delta < 0:
        verdict = "llse_cmd20_gate_candidate_improves"
        next_action = (
            "inspect LLSE cmd20 gated candidate "
            f"{best.get('variant_id', '')}; score delta {best_delta:.4f}"
        )
    elif best:
        verdict = "llse_cmd20_gate_no_improvement"
        next_action = (
            "profile LLSE marker/opcode semantics beyond cmd20 gates; "
            f"best gate {best.get('gate_policy', '')} delta {best_delta:.4f}"
        )
    else:
        verdict = "llse_cmd20_gate_no_rows"
        next_action = "review LLSE cmd20 gate probe inputs"
    return {
        "scope": "total",
        ...
        "best_gate_policy": best.get("gate_policy", ""),
        "best_noncmd20_policy": best.get("noncmd20_policy", ""),
        "best_smallxy_policy": best.get("smallxy_policy", ""),
        "best_cmd20_seen": best.get("cmd20_seen", ""),
        "best_cmd20_applied": best.get("cmd20_applied", ""),
        "best_cmd20_ignored": best.get("cmd20_ignored", ""),
...
def render_cards(rows: list[dict[str, str]], output_dir: Path, limit: int = 24) -> str:
    ...
    for row in rows[:limit]:
        ...
        cards.append(
            ...
            f"applied {html.escape(row.get('cmd20_applied', ''))}/"
            f"{html.escape(row.get('cmd20_seen', ''))}<br>"
...
<header>
  ...
  <div class="muted">LLSE cmd20 gate policies against the broad high-arg2 candidate.</div>
  <p>{links}</p>
</header>
<main>
  <section class="grid">
    <div class="stat"><div class="label">Variants</div><div class="value">{html.escape(summary['variant_rows'])}</div></div>
    <div class="stat"><div class="label">Best Gate</div><div class="value">{html.escape(summary['best_gate_policy'])}</div></div>
...
<script type="application/json" id="llse-cmd20-gate-data">{html.escape(data_json)}</script>
</body>
</html>
"""
...
def write_report(args: argparse.Namespace) -> tuple[dict[str, str], list[dict[str, str]]]:
    ...
    tuple_whitelists = load_tuple_whitelists(args.cmd20_tuples, args.top8_limit, args.top32_limit)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Test gated LLSE cmd20 semantics.")
    parser.add_argument("-o", "--output", type=Path, default=DEFAULT_OUTPUT)
    parser.add_argument("--segments", type=Path, default=DEFAULT_SEGMENTS)
    parser.add_argument("--higharg2-summary", type=Path, default=DEFAULT_HIGHARG2_SUMMARY)
    parser.add_argument("--cmd20-tuples", type=Path, default=DEFAULT_CMD20_TUPLES)
    ...
    parser.add_argument("--top8-limit", type=int, default=8)
    parser.add_argument("--top32-limit", type=int, default=32)
    parser.add_argument("--title", default="Lands of Lore II .tex LLSE cmd20 Gate Probe")
    ...
    print(f"Best gate: {summary['best_gate_policy']}")
    print(f"Best applied cmd20: {summary['best_cmd20_applied']}/{summary['best_cmd20_seen']}")
