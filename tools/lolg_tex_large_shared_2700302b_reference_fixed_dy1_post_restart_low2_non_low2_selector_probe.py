#!/usr/bin/env python3
"""Derive a high3 non-low2 selector after restart-low2 replay for shared 0x2700302b."""
...
DEFAULT_OUTPUT = Path("output/tex_large_shared_2700302b_reference_fixed_dy1_post_restart_low2_non_low2_selector_probe")
...
DEFAULT_SOURCE_SUMMARY = Path(
    "output/tex_large_shared_2700302b_reference_fixed_dy1_post_restart_low2_non_low2_source_probe/summary.csv"
)
...
def non_low2_target(row: dict[str, str]) -> bool:
    return row.get("residual_kind") == "small_delta" and int_value(row, "delta_signed") in NON_LOW2_DELTAS
...
def high3_non_low2_source(segment: bytes) -> list[dict[str, int]]:
    rows: list[dict[str, int]] = []
    source_index = 0
    for segment_offset, value in enumerate(segment):
        transformed = mapping_probe.signed_bits(value >> 5, 3)
        if signed_byte(transformed) not in NON_LOW2_DELTAS:
            continue
        rows.append(
            {
...
def build_summary(
    ...
    *,
    ...
    non_low2_target_pixels = int_value(miss_summary, "low2_impossible_pixels")
    low2_rep = int_value(miss_summary, "low2_representable_pixels")
    source_best_lcs = int_value(source_summary, "best_lcs_pixels")
    source_gap = source_best_lcs - len(alignment_rows)
    target_gap = len(target_rows) - non_low2_target_pixels
    ...
    issue_rows = len(issues) + (1 if target_gap else 0) + (1 if source_gap else 0)
    segment_offsets = [int_value(row, "segment_offset") for row in alignment_rows]

    if issue_rows:
        verdict = "shared_2700302b_reference_fixed_dy1_post_restart_low2_non_low2_selector_issues"
        next_action = "fix shared 0x2700302b post-restart high3 non-low2 selector inputs"
    elif len(alignment_rows) / non_low2_target_pixels >= 0.75 if non_low2_target_pixels else False:
        verdict = "shared_2700302b_reference_fixed_dy1_post_restart_low2_non_low2_selector_ready"
        next_action = (
            "measure guarded high3 non-low2 replay after restart low2 for shared 0x2700302b; "
            f"selector adds {len(alignment_rows)} pixels"
        )
    else:
        verdict = "shared_2700302b_reference_fixed_dy1_post_restart_low2_non_low2_selector_weak"
        next_action = (
            "search alternate selector for post-restart high3 non-low2 deltas in shared 0x2700302b; "
            f"selector covers {len(alignment_rows)}/{non_low2_target_pixels}"
        )
    ...
    return {
        ...
        "source_transform": "high3_non_low2_only",
        ...
        "non_low2_target_pixels": str(non_low2_target_pixels),
        "low2_representable_pixels": str(low2_rep),
        "aligned_pixels": str(len(alignment_rows)),
        "aligned_ratio": float_text(len(alignment_rows) / non_low2_target_pixels if non_low2_target_pixels else 0.0),
        ...
        "source_best_id": source_summary.get("best_source_id", ""),
        ...
        "non_low2_selector_verdict": verdict,
...
def write_report(
    ...
    source_rows = high3_non_low2_source(segment)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Derive a high3 non-low2 selector after restart-low2 replay.")
    ...
    parser.add_argument("--title", default="Lands of Lore II .tex Shared 0x2700302b Post-restart High3 Non-low2 Selector")
    ...
    print(f"Verdict: {summary['non_low2_selector_verdict']}")
