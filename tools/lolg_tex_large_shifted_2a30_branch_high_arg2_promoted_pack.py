#!/usr/bin/env python3
"""Pack reviewed shifted 0x2a30 branch high-arg2 preview as coverage assets."""
...
DEFAULT_OUTPUT = Path("output/tex_large_shifted_2a30_branch_high_arg2_promoted_pack")
DEFAULT_PREVIEW_REVIEW = Path("output/tex_large_shifted_2a30_branch_route_previews_review/review.csv")
...
def build_manifest(output_dir: Path, review_rows: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    for review in sorted(review_rows, key=lambda row: (row.get("archive_tag", ""), row.get("pcx_name", "").lower())):
        ...
        if review.get("renderer_mode") != EXPECTED_RENDERER_MODE:
            issues.append("renderer_mode_mismatch")
        if int_text(review.get("high_arg2_skips")) <= 0:
        ...
        rows.append(
            {
                ...
                "renderer_mode": review.get("renderer_mode", ""),
                "renderer_start": review.get("renderer_start", ""),
                "renderer_width": review.get("renderer_width", ""),
                "renderer_height": review.get("renderer_height", ""),
                "renderer_score": review.get("renderer_score", ""),
                "high_arg2_skips": review.get("high_arg2_skips", ""),
                "markerknown_skips": review.get("markerknown_skips", ""),
...
def build_summary(review_rows: list[dict[str, str]], manifest_rows: list[dict[str, str]]) -> dict[str, str]:
    ...
    clean = len(review_rows) == 1 and len(ready_rows) == 1 and len(eligible_rows) == 1 and not issue_rows
    if clean:
        verdict = "branch_high_arg2_promoted_pack_ready"
        next_action = "refresh .tex augmented coverage with branch high-arg2 promotion"
    elif issue_rows:
        verdict = "branch_high_arg2_promoted_pack_blocked"
        next_action = "fix branch high-arg2 promoted pack issues"
    else:
        verdict = "branch_high_arg2_promoted_pack_incomplete"
        next_action = "complete branch high-arg2 promoted pack"
    return {
        "scope": "total",
        ...
        "high_arg2_assets": str(sum(1 for row in eligible_rows if int_text(row.get("high_arg2_skips")) > 0)),
        "expected_mode_assets": str(
            sum(1 for row in eligible_rows if row.get("renderer_mode") == EXPECTED_RENDERER_MODE)
        ),
...
<article>
  ...
  <div class="body">
    ...
    <div class="muted">coverage {html.escape(row.get('coverage_eligible', ''))} / high-arg2 {html.escape(row.get('high_arg2_skips', ''))}</div>
...
<main class="wrap">
  <section class="stats">
    ...
    <div class="stat"><div class="label">High-arg2 assets</div><div class="value">{html.escape(summary['high_arg2_assets'])}</div></div>
...
<script>const TEX_LARGE_SHIFTED_2A30_BRANCH_HIGH_ARG2_PROMOTED_PACK = {data_json};</script>
</body>
</html>
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Pack reviewed shifted 0x2a30 branch high-arg2 preview.")
    parser.add_argument("-o", "--output", type=Path, default=DEFAULT_OUTPUT)
    parser.add_argument("--preview-review", type=Path, default=DEFAULT_PREVIEW_REVIEW)
    parser.add_argument(
        "--title",
        default="Lands of Lore II .tex Large Shifted 2a30 Branch High-Arg2 Promoted Pack",
    )
    ...
    print(f"High-arg2 assets: {summary['high_arg2_assets']}")
