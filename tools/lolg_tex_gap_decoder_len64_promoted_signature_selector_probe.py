#!/usr/bin/env python3
"""Score source-side selectors for any remaining post-medium8 zero signature."""
...
DEFAULT_OUTPUT = Path("output/tex_gap_decoder_len64_promoted_signature_selector_probe")
...
DEFAULT_TARGET_SIGNATURE = "trailing|large32|left_nonzero|right_nonzero"
DEFAULT_GREEDY_MIN_TARGET_ROWS = 2
...
def inferred_bucket(signature: str) -> str:
    parts = signature.split("|")
    return parts[1] if len(parts) > 1 else ""


def normalized_pair_family(target_bucket: str, pair_family: str) -> str:
    return pair_family or f"{target_bucket}_pair"
...
def candidate_keys(
    ...
    if target_bucket and bucket_feature in features:
        non_bucket_features = [feature for feature in features if feature != bucket_feature]
        for feature in features:
            if feature == bucket_feature:
                continue
            keys.append((pair_family, tuple(sorted((bucket_feature, feature)))))
        if max_combo_size >= 3:
            triple_family = f"{pair_family}_triple"
            for left, right in combinations(non_bucket_features, 2):
                keys.append((triple_family, tuple(sorted((bucket_feature, left, right)))))
    return keys
...
def build_candidates(
    ...
    for operation in operation_rows:
        for key in candidate_keys(operation, target_bucket, pair_family, max_combo_size):
...
def build_rows(
    ...
    summary = {
        ...
        "max_combo_size": str(max_combo_size),
        "greedy_families": ";".join(sorted(greedy_families)),
...
<header>
  <div class="wrap">
    ...
    <div class="sub">Scores source-side selectors for one post-medium8 zero signature.</div>
  </div>
</header>
<main class="wrap">
  <section class="stats">
    <div class="stat"><div class="label">Target signature</div><div class="value">{html.escape(summary['target_signature'])}</div></div>
...
<script>
const TEX_GAP_DECODER_LEN64_PROMOTED_SIGNATURE_SELECTOR_PROBE = {data_json};
</script>
</body>
</html>
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Score source-side selectors for a remaining post-medium8 .tex zero signature."
    )
    parser.add_argument("--targets", type=Path, default=DEFAULT_TARGETS)
    parser.add_argument("--operations", type=Path, default=DEFAULT_OPERATIONS)
    parser.add_argument("--target-signature", default=DEFAULT_TARGET_SIGNATURE)
    parser.add_argument("--target-bucket", default="")
    parser.add_argument("--pair-family", default="")
    parser.add_argument("--max-combo-size", type=int, choices=(2, 3), default=2)
    parser.add_argument(
        "--greedy-include-triples",
        action="store_true",
        help="Allow bucket+two-feature selectors in the greedy union.",
    )
    parser.add_argument("--greedy-min-target-rows", type=int, default=DEFAULT_GREEDY_MIN_TARGET_ROWS)
    parser.add_argument("-o", "--output", type=Path, default=DEFAULT_OUTPUT)
    parser.add_argument("--title", default="")
    args = parser.parse_args()

    target_bucket = args.target_bucket or inferred_bucket(args.target_signature)
    pair_family = normalized_pair_family(target_bucket, args.pair_family)
    greedy_families = {pair_family}
    if args.greedy_include_triples:
        greedy_families.add(f"{pair_family}_triple")
    title = (
        args.title
        or "Lands of Lore II .tex Len64 Promoted Signature Selector Probe"
    )

    summary, candidates, greedy, targets = build_rows(
        read_csv(args.targets),
        read_csv(args.operations),
        args.target_signature,
        target_bucket,
        pair_family,
        args.greedy_min_target_rows,
        args.max_combo_size,
        greedy_families,
    )
    ...
    html_path.write_text(build_html(summary, candidates, greedy, targets, args.output, title))

    print(f"Target signature: {summary['target_signature']}")
