#!/usr/bin/env python3
"""Queue zero runs after the promoted len64 run probe."""
...
DEFAULT_OUTPUT = Path("output/tex_gap_decoder_len64_promoted_zero_queue")
DEFAULT_SPANS = Path("output/tex_gap_decoder_len64_promoted_run_probe/by_span.csv")
DEFAULT_RUNS = Path("output/tex_gap_decoder_len64_promoted_run_probe/runs.csv")
...
def build_html(
    ...
    links = " ".join(
        ...
        for label, path in (
            ...
            ("queue.csv", "queue.csv"),
            ("by_signature.csv", "by_signature.csv"),
            ("by_fixture.csv", "by_fixture.csv"),
        )
    )
...
<header>
  <div class="wrap">
    ...
    <div class="sub">Ranks remaining zero runs after the len64-promoted replay by position and neighbor context.</div>
  </div>
</header>
<main class="wrap">
  <section class="stats">
...
<script>
const TEX_GAP_DECODER_LEN64_PROMOTED_ZERO_QUEUE = {data_json};
</script>
</body>
</html>
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Queue .tex zero runs after the promoted len64 replay."
    )
    parser.add_argument("--spans", type=Path, default=DEFAULT_SPANS)
    parser.add_argument("--runs", type=Path, default=DEFAULT_RUNS)
    parser.add_argument("--fixtures", type=Path, default=DEFAULT_FIXTURES)
    parser.add_argument("-o", "--output", type=Path, default=DEFAULT_OUTPUT)
    parser.add_argument("--title", default="Lands of Lore II .tex Len64 Promoted Zero Queue")
    ...
    html_path.write_text(build_html(summary, queue_rows, signature_rows, by_fixture_rows, args.title))
