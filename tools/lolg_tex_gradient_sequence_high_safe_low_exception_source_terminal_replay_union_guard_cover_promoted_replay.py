#!/usr/bin/env python3
"""Promote compact guarded replay-union source-terminal roots into fixture buffers."""
...
DEFAULT_OUTPUT = Path(
    "output/tex_gradient_sequence_high_safe_low_exception_source_terminal_replay_union_guard_cover_promoted_replay"
)
...
DEFAULT_DEPENDENCY_SLOTS = Path("output/tex_gradient_sequence_high_safe_low_exception_source_dependency/slots.csv")
DEFAULT_GUARD_ROOTS = Path(
...
def selected_guard_slots(
    dependency_slot_rows: list[dict[str, str]],
    guard_root_rows: list[dict[str, str]],
) -> list[dict[str, str]]:
    slots_by_rank = {row["rank"]: row for row in dependency_slot_rows}
    output: list[dict[str, str]] = []
    for guard in guard_root_rows:
        if guard.get("verdict") != "correct":
            continue
        if not guard.get("selected_contexts"):
            continue
        slot = dict(slots_by_rank.get(guard.get("root_slot_rank", ""), {}))
        if not slot:
            continue
        prediction = guard.get("prediction", "")
        slot["guard_prediction_low"] = prediction
        slot["guard_predicted_byte"] = f"{slot.get('target_high', '')}{prediction}" if prediction else ""
        slot["guard_selected_contexts"] = guard.get("selected_contexts", "")
        slot["guard_root_verdict"] = guard.get("verdict", "")
        output.append(slot)
    output.sort(key=lambda row: (archive_key(row), int_value(row, "target_offset"), int_value(row, "rank")))
    return output


def grouped_guard_slots(rows: list[dict[str, str]]) -> dict[tuple[str, str, str], list[dict[str, str]]]:
    grouped: dict[tuple[str, str, str], list[dict[str, str]]] = defaultdict(list)
    for row in rows:
        if not row.get("guard_predicted_byte"):
            continue
...
def build_rows(
    *,
    ...
    slot_groups = grouped_guard_slots(selected_guard_slots(dependency_slot_rows, guard_root_rows))
    ...
    for base_fixture in sorted(base_fixture_rows, key=lambda row: fixture_sort_key(archive_key(row))):
        ...
        rejected_key = (base_fixture.get("rank", ""), key[1], key[2])
        ...
        for slot in slot_groups.get(key, []):
            ...
            offset = int_value(slot, "target_offset")
            predicted = parse_hex_byte(slot.get("guard_predicted_byte", ""), target_issues, "predicted_byte")
            target = parse_hex_byte(slot.get("target_byte", ""), target_issues, "target_byte")
            ...
            guard_overlap = overlap_bytes(guard_mask, offset, offset + 1)
            ...
            if slot.get("guard_root_verdict") != "correct":
                target_issues.append("guard_root_not_correct")
            if predicted is not None and target is not None and predicted != target:
                target_issues.append("target_prediction_mismatch")
            if predicted is not None and expected_value is not None and predicted != expected_value:
                target_issues.append("guard_would_write_false_byte")
            ...
            if guard_overlap:
                target_issues.append("guard_overlap")

            false_bytes = 1 if "guard_would_write_false_byte" in target_issues else 0
            added_bytes = 0
            exact_bytes = 0
            skipped_known = 0
            skipped_rejected = 0
            if not target_issues:
                if base_overlap:
                    ...
                    guard_mask[offset] = 0xFF
                    added_bytes = 1
                    exact_bytes = 1
            else:
                issue_rows += 1

            stats["guard_candidate_rows"] += 1
            stats["guard_added_bytes"] += added_bytes
            stats["guard_exact_bytes"] += exact_bytes
            stats["guard_false_bytes"] += false_bytes
            ...
            promotion_rows.append(
                {
                    ...
                    "relative_offset": slot.get("relative_offset", ""),
                    "expected_byte": expected_byte,
                    "target_byte": slot.get("target_byte", ""),
                    "predicted_byte": slot.get("guard_predicted_byte", ""),
                    "selected_contexts": slot.get("guard_selected_contexts", ""),
                    ...
                    "guard_added_bytes": str(added_bytes),
                    "guard_exact_bytes": str(exact_bytes),
                    "guard_false_bytes": str(false_bytes),
        ...
        decoded_path = fixture_output_dir / f"{stem}_decoded_guard_cover.bin"
        known_mask_path = fixture_output_dir / f"{stem}_known_mask.bin"
        guard_mask_path = fixture_output_dir / f"{stem}_guard_cover_mask.bin"
        decoded_path.write_bytes(decoded)
        known_mask_path.write_bytes(known_mask)
        guard_mask_path.write_bytes(guard_mask)
        native_preview_path = native_preview_dir / f"{stem}_guard_cover.png"
        fullhd_preview_path = fullhd_preview_dir / f"{stem}_guard_cover_fullhd.png"
        fullhd_width, fullhd_height = render_preview(
            expected=expected,
            decoded=bytes(decoded),
            known_mask=bytes(known_mask),
            risk_mask=bytes(guard_mask),
        ...
        output_fixture_rows.append(
            {
                ...
                "guard_candidate_rows": str(stats["guard_candidate_rows"]),
                ...
                "guard_mask_path": guard_mask_path.as_posix(),
    ...
    guard_added = sum(int_value(row, "guard_added_bytes") for row in output_fixture_rows)
    guard_false = sum(int_value(row, "guard_false_bytes") for row in output_fixture_rows)
    summary = {
        "scope": "total",
        "fixture_rows": str(len(output_fixture_rows)),
        "guard_candidate_rows": str(len(promotion_rows)),
        "promoted_rows": str(sum(1 for row in promotion_rows if int_value(row, "guard_added_bytes"))),
        "base_clean_bytes": str(sum(int_value(row, "base_clean_bytes") for row in output_fixture_rows)),
        "guard_added_bytes": str(guard_added),
        "guard_exact_bytes": str(sum(int_value(row, "guard_exact_bytes") for row in output_fixture_rows)),
        "guard_false_bytes": str(guard_false),
        ...
        "promotion_ready_bytes": str(guard_added if issue_rows == 0 and guard_false == 0 else 0),
...
<div class="grid">
  <div class="box"><div class="num">{summary['guard_added_bytes']}</div><div class="muted">guard added bytes</div></div>
  <div class="box"><div class="num">{summary['promoted_rows']}/{summary['guard_candidate_rows']}</div><div class="muted">promoted rows</div></div>
  <div class="box"><div class="num">{summary['guard_false_bytes']}</div><div class="muted">false bytes</div></div>
...
<script type="application/json" id="gradient-guard-cover-promoted-data">{html.escape(data_json)}</script>
</body>
</html>
"""
...
def main() -> None:
    ...
    parser.add_argument("--dependency-slots", type=Path, default=DEFAULT_DEPENDENCY_SLOTS)
    parser.add_argument("--guard-roots", type=Path, default=DEFAULT_GUARD_ROOTS)
    parser.add_argument(
        "--title",
        default="Lands of Lore II .tex Gradient Source Terminal Guard Cover Promoted Replay",
    )
    ...
    summary, fixture_rows, promotion_rows = build_rows(
        ...
        dependency_slot_rows=read_csv(args.dependency_slots),
        guard_root_rows=read_csv(args.guard_roots),
    )
    ...
    print(f"Promoted rows: {summary['promoted_rows']}/{summary['guard_candidate_rows']}")
    print(f"Added guard bytes: {summary['guard_added_bytes']}")
    print(f"False bytes: {summary['guard_false_bytes']}")
