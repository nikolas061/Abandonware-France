#!/usr/bin/env python3
"""Measure guarded high3 non-low2 replay coverage for shared 0x2700302b."""
...
DEFAULT_OUTPUT = Path(
    "output/tex_large_shared_2700302b_reference_fixed_dy1_post_restart_low2_non_low2_guarded_replay_probe"
)
...
DEFAULT_GUARDED_RESTART_SELECTED_PIXELS = Path(
    "output/tex_large_shared_2700302b_reference_fixed_dy1_post_low2_restart_low2_guarded_replay_probe/selected_pixels.csv"
)
DEFAULT_NON_LOW2_SELECTOR_SUMMARY = Path(
    "output/tex_large_shared_2700302b_reference_fixed_dy1_post_restart_low2_non_low2_selector_probe/summary.csv"
)
DEFAULT_NON_LOW2_SELECTED_PIXELS = Path(
    "output/tex_large_shared_2700302b_reference_fixed_dy1_post_restart_low2_non_low2_selector_probe/selected_pixels.csv"
)
...
SUMMARY_FIELDNAMES = [
    ...
    "selected_pixel_rows",
    "overlap_restart_high3_pixels",
    "high3_selector_gap_pixels",
    "accounting_gap_pixels",
    "issue_rows",
    "guarded_high3_replay_verdict",
    "next_action",
]
...
def build_selected_rows(
    ...
    for row in restart_rows:
        rows.append(
            {
                "rank": str(len(rows) + 1),
                "phase": row.get("phase", "restart_replay"),
...
def build_summary(
    ...
    target_pixels = int_value(restart_summary, "target_pixels")
    target_nonzero = int_value(restart_summary, "target_nonzero_pixels")
    current_covered = int_value(restart_summary, "combined_covered_pixels")
    current_remaining = int_value(restart_summary, "remaining_nonzero_pixels")
    high3_aligned = int_value(selector_summary, "aligned_pixels")
    high3_selector_gap = high3_aligned - len(high3_rows)
    restart_coords = {coord(row) for row in restart_rows}
    high3_coords = {coord(row) for row in high3_rows}
    overlap = len(restart_coords & high3_coords)
    combined_selected = len(restart_rows) + len(high3_rows)
    combined_covered = min(target_pixels, current_covered + high3_aligned)
    remaining = max(0, current_remaining - high3_aligned)
    accounting_gap = combined_selected - (
        int_value(restart_summary, "selected_pixel_rows") + high3_aligned
    )
    issue_rows = len(issues)
    if overlap:
        issue_rows += 1
    if high3_selector_gap:
        issue_rows += 1
    if accounting_gap:
        issue_rows += 1

    if issue_rows:
        verdict = "shared_2700302b_reference_fixed_dy1_post_restart_low2_non_low2_guarded_replay_issues"
        next_action = "fix shared 0x2700302b high3 non-low2 guarded replay inputs"
    elif combined_covered / target_pixels >= 0.83 if target_pixels else False:
        verdict = "shared_2700302b_reference_fixed_dy1_post_restart_low2_non_low2_guarded_replay_ready"
        next_action = (
            "profile post-high3 residual producer for shared 0x2700302b frontier "
            f"{restart_summary.get('frontier_id', '')}; {remaining} nonzero pixels remain"
        )
    else:
        verdict = "shared_2700302b_reference_fixed_dy1_post_restart_low2_non_low2_guarded_replay_partial"
        next_action = (
            "seek stronger high3 non-low2 replay for shared 0x2700302b; "
            f"{remaining} nonzero pixels remain"
        )

    return {
        "scope": "total",
        "archive": restart_summary.get("archive", ""),
        "archive_tag": restart_summary.get("archive_tag", ""),
        "pcx_name": restart_summary.get("pcx_name", ""),
        "frontier_id": restart_summary.get("frontier_id", ""),
        "dy": restart_summary.get("dy", ""),
        "shift": restart_summary.get("shift", ""),
        "target_pixels": str(target_pixels),
        "target_zero_pixels": restart_summary.get("target_zero_pixels", "0"),
        "target_nonzero_pixels": str(target_nonzero),
        "dy1_zero_plus_pixels": restart_summary.get("dy1_zero_plus_pixels", "0"),
        "fixed_residual_nonzero_pixels": restart_summary.get("fixed_residual_nonzero_pixels", "0"),
        "initial_low2_aligned_pixels": restart_summary.get("initial_low2_aligned_pixels", "0"),
        "restart_low2_aligned_pixels": restart_summary.get("restart_low2_aligned_pixels", "0"),
        "high3_non_low2_aligned_pixels": str(high3_aligned),
        "combined_selected_pixels": str(combined_selected),
        ...
        "selected_pixel_rows": str(combined_selected),
        "overlap_restart_high3_pixels": str(overlap),
        "high3_selector_gap_pixels": str(high3_selector_gap),
        "accounting_gap_pixels": str(accounting_gap),
        "issue_rows": str(issue_rows),
        "guarded_high3_replay_verdict": verdict,
...
<div class="stats">
  ...
  <div class="stat"><div class="label">High3 Added</div><div class="value">{html.escape(summary['high3_non_low2_aligned_pixels'])}</div></div>
...
def write_report(args: argparse.Namespace) -> tuple[dict[str, str], list[dict[str, str]]]:
    ...
    issues: list[str] = []
    restart_summary = read_summary(args.guarded_restart_summary)
    selector_summary = read_summary(args.non_low2_selector_summary)
    if not restart_summary:
        issues.append("missing_guarded_restart_summary")
    if not selector_summary:
        issues.append("missing_non_low2_selector_summary")
    restart_rows = read_csv(args.guarded_restart_selected_pixels)
    high3_rows = read_csv(args.non_low2_selected_pixels)
    if not restart_rows:
        issues.append("missing_guarded_restart_selected_pixels")
    if not high3_rows:
        issues.append("missing_non_low2_selected_pixels")
    selected_rows = build_selected_rows(restart_rows, high3_rows)
    summary = build_summary(restart_summary, selector_summary, restart_rows, high3_rows, issues)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Measure guarded high3 non-low2 replay coverage for shared 0x2700302b.")
    parser.add_argument("-o", "--output", type=Path, default=DEFAULT_OUTPUT)
    parser.add_argument("--guarded-restart-summary", type=Path, default=DEFAULT_GUARDED_RESTART_SUMMARY)
    parser.add_argument(
        "--guarded-restart-selected-pixels",
        type=Path,
        default=DEFAULT_GUARDED_RESTART_SELECTED_PIXELS,
    )
    parser.add_argument("--non-low2-selector-summary", type=Path, default=DEFAULT_NON_LOW2_SELECTOR_SUMMARY)
    parser.add_argument("--non-low2-selected-pixels", type=Path, default=DEFAULT_NON_LOW2_SELECTED_PIXELS)
    parser.add_argument(
        "--title",
        default="Lands of Lore II .tex Shared 0x2700302b High3 Non-low2 Guarded Replay",
    )
    ...
    print(f"Verdict: {summary['guarded_high3_replay_verdict']}")
