#!/usr/bin/env python3
"""Profile residual pixels after guarded large-delta replay for shared 0x2700302b."""
...
DEFAULT_OUTPUT = Path("output/tex_large_shared_2700302b_reference_fixed_dy1_post_large_delta_residual_profile_probe")
DEFAULT_GUARDED_LARGE_DELTA_SUMMARY = Path(
...
SUMMARY_FIELDNAMES = [
    ...
    "guarded_large_delta_remaining_nonzero_pixels",
    "guarded_large_delta_gap_pixels",
    "issue_rows",
    "post_large_delta_residual_verdict",
    "next_action",
]
...
def build_summary(
    ...
    if issue_rows:
        verdict = "shared_2700302b_reference_fixed_dy1_post_large_delta_residual_profile_issues"
        next_action = "fix shared 0x2700302b post-large-delta residual profile inputs"
    elif len(small) >= len(large):
        verdict = "shared_2700302b_reference_fixed_dy1_post_large_delta_small_delta_remaining"
        next_action = (
            "split post-large-delta residual small-delta misses for shared 0x2700302b frontier "
            f"{guarded_summary.get('frontier_id', '')}; {len(small)} small-delta and {len(large)} large-delta pixels remain"
        )
    else:
        verdict = "shared_2700302b_reference_fixed_dy1_post_large_delta_large_delta_remaining"
        next_action = (
            "derive next large-delta producer after large-delta replay for shared 0x2700302b frontier "
    ...
    return {
        ...
        "guarded_large_delta_remaining_nonzero_pixels": str(guarded_remaining),
        "guarded_large_delta_gap_pixels": str(guarded_gap),
        "issue_rows": str(issue_rows),
        "post_large_delta_residual_verdict": verdict,
...
def write_report(args: argparse.Namespace) -> tuple[dict[str, str], list[dict[str, str]]]:
    ...
    issues: list[str] = []
    guarded_summary = read_summary(args.guarded_large_delta_summary)
    if not guarded_summary:
        issues.append("missing_guarded_large_delta_summary")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Profile post-large-delta residuals for shared 0x2700302b.")
    parser.add_argument("-o", "--output", type=Path, default=DEFAULT_OUTPUT)
    parser.add_argument("--guarded-large-delta-summary", type=Path, default=DEFAULT_GUARDED_LARGE_DELTA_SUMMARY)
    ...
    parser.add_argument(
        "--title",
        default="Lands of Lore II .tex Shared 0x2700302b Post-large-delta Residual Profile",
    )
    ...
    print(f"Verdict: {summary['post_large_delta_residual_verdict']}")
