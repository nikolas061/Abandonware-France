#!/usr/bin/env python3
"""Integrate context-split high-row residual corrections into clean fixtures."""
...
SUMMARY_FIELDNAMES = [
    ...
    "promoted_rows",
    "base_clean_bytes",
    "residual_candidate_bytes",
    "residual_added_bytes",
    "residual_exact_bytes",
    "residual_false_bytes",
    "skipped_known_bytes",
...
def promoted_targets(rows: list[dict[str, str]]) -> dict[tuple[str, str, str], list[dict[str, str]]]:
    grouped: dict[tuple[str, str, str], list[dict[str, str]]] = defaultdict(list)
    for row in rows:
        if row.get("residual_promoted") == "1":
...
def build_rows(
    *,
    ...
    target_groups = promoted_targets(selector_rows)
    ...
    for base_fixture in sorted(base_fixture_rows, key=lambda row: fixture_sort_key(archive_key(row))):
        ...
        for target in target_groups.get(key, []):
            ...
            offset = int_field(target, "source_offset", -1)
            predicted = parse_hex_byte(target.get("residual_value_hex", ""), target_issues, "residual_value_hex")
            ...
            residual_overlap = overlap_bytes(residual_mask, offset, offset + 1)
            ...
            if predicted is not None and expected_value is not None and predicted != expected_value:
                target_issues.append("residual_would_write_false_byte")
            ...
            if residual_overlap:
                target_issues.append("residual_overlap")

            false_bytes = 1 if "residual_would_write_false_byte" in target_issues else 0
            added_bytes = 0
            exact_bytes = 0
            skipped_known = 0
            skipped_rejected = 0
            if not target_issues:
                if base_overlap:
                    ...
                    residual_mask[offset] = 0xFF
                    added_bytes = 1
                    exact_bytes = 1
            else:
                issue_rows += 1

            stats["residual_candidate_bytes"] += 1
            stats["residual_added_bytes"] += added_bytes
            stats["residual_exact_bytes"] += exact_bytes
            stats["residual_false_bytes"] += false_bytes
            ...
            promotion_rows.append(
                {
                    ...
                    "byte_index": target.get("byte_index", ""),
                    "expected_byte": expected_text,
                    "predicted_byte": target.get("residual_value_hex", ""),
                    "residual_delta": target.get("residual_delta", ""),
                    "guard_key": target.get("residual_guard_key", ""),
                    "base_known_overlap_bytes": str(base_overlap),
                    "residual_overlap_bytes": str(residual_overlap),
                    "rejected_overlap_bytes": str(rejected_overlap),
                    "residual_added_bytes": str(added_bytes),
                    "residual_exact_bytes": str(exact_bytes),
                    "residual_false_bytes": str(false_bytes),
        ...
        decoded_path = fixture_output_dir / f"{stem}_decoded_high_row_residual.bin"
        known_mask_path = fixture_output_dir / f"{stem}_known_mask.bin"
        residual_mask_path = fixture_output_dir / f"{stem}_high_row_residual_mask.bin"
        decoded_path.write_bytes(decoded)
        known_mask_path.write_bytes(known_mask)
        residual_mask_path.write_bytes(residual_mask)
        native_preview_path = native_preview_dir / f"{stem}_high_row_residual.png"
        fullhd_preview_path = fullhd_preview_dir / f"{stem}_high_row_residual_fullhd.png"
        fullhd_width, fullhd_height = render_preview(
            expected=expected,
            decoded=bytes(decoded),
            known_mask=bytes(known_mask),
            risk_mask=bytes(residual_mask),
        ...
        output_fixture_rows.append(
            {
                ...
                "residual_target_rows": str(len(target_groups.get(key, []))),
                "residual_candidate_bytes": str(stats["residual_candidate_bytes"]),
                "residual_added_bytes": str(stats["residual_added_bytes"]),
                "residual_exact_bytes": str(stats["residual_exact_bytes"]),
                "residual_false_bytes": str(stats["residual_false_bytes"]),
                ...
                "residual_mask_path": residual_mask_path.as_posix(),
    ...
    residual_added = sum(int_value(row, "residual_added_bytes") for row in output_fixture_rows)
    residual_false = sum(int_value(row, "residual_false_bytes") for row in output_fixture_rows)
    ...
    summary = {
        ...
        "promoted_rows": str(sum(1 for row in promotion_rows if int_field(row, "residual_added_bytes") > 0)),
        "base_clean_bytes": str(sum(int_field(row, "base_clean_bytes") for row in output_fixture_rows)),
        "residual_candidate_bytes": str(sum(int_field(row, "residual_candidate_bytes") for row in output_fixture_rows)),
        "residual_added_bytes": str(residual_added),
        "residual_exact_bytes": str(sum(int_field(row, "residual_exact_bytes") for row in output_fixture_rows)),
        "residual_false_bytes": str(residual_false),
        ...
        "promotion_ready_bytes": str(residual_added if issue_count == 0 and residual_false == 0 else 0),
        "issue_rows": str(issue_count),
        "review_verdict": (
            "frontier80_prior_high_row_exact_residual_context_split_fixture_replay_ready"
            if issue_count == 0 and residual_false == 0 and residual_added > 0
            else "frontier80_prior_high_row_exact_residual_context_split_fixture_replay_weak"
        ),
        "next_probe": (
            "rerun Frontier80 clean-gap queue with context-split residual fixture base"
            if issue_count == 0 and residual_false == 0 and residual_added > 0
            else "review context-split residual fixture integration issues"
        ),
    }
...
<div class="grid">
  <div class="box"><div class="num">{summary['residual_added_bytes']}</div><div class="muted">residual added bytes</div></div>
  <div class="box"><div class="num">{summary['promoted_rows']}/{summary['target_rows']}</div><div class="muted">promoted targets</div></div>
  <div class="box"><div class="num">{summary['residual_false_bytes']}</div><div class="muted">false bytes</div></div>
...
<script type="application/json" id="high-row-residual-fixture-replay-data">{html.escape(data_json)}</script>
</body>
</html>
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--title",
        default="Lands of Lore II .tex Frontier80 Prior High Row Exact Residual Context Split Fixture Replay",
    )
