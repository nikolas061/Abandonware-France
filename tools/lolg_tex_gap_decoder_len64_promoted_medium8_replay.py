#!/usr/bin/env python3
"""Replay false-free medium8 selector targets on top of large32-promoted buffers."""
...
DEFAULT_OUTPUT = Path("output/tex_gap_decoder_len64_promoted_medium8_replay")
DEFAULT_FIXTURES = Path("output/tex_gap_rule_fixtures/manifest.csv")
DEFAULT_FRONTIERS = Path("output/tex_gap_frontier_report/frontiers.csv")
DEFAULT_BASE_FIXTURES = Path("output/tex_gap_decoder_len64_promoted_large32_replay/fixtures.csv")
DEFAULT_CLEAN_DECISIONS = Path("output/tex_gap_decoder_clean_replay/decisions.csv")
DEFAULT_SELECTOR_TARGETS = Path("output/tex_gap_decoder_len64_promoted_medium8_selector_probe/targets.csv")
    ...
    *,
    ...
    for base_fixture in sorted(base_fixture_rows, key=lambda row: fixture_sort_key(fixture_key(row))):
        ...
        for target in sorted(
            ...
            key=lambda row: (int_value(row, "start"), int_value(row, "op_index")),
        ):
            ...
            if target.get("length_bucket") != TARGET_BUCKET:
                target_issues.append("target_not_medium8")
            if target.get("signature") != TARGET_SIGNATURE:
        ...
        decoded_path = fixture_output_dir / f"{stem}_decoded_medium8_promoted.bin"
        ...
        native_preview_path = native_preview_dir / f"{stem}_medium8_promoted.png"
        fullhd_preview_path = fullhd_preview_dir / f"{stem}_medium8_promoted_fullhd.png"
...
<article class="card">
  ...
  <div class="card-body">
    ...
    <div class="muted">+{html.escape(row.get('selector_added_bytes', ''))} medium8 - {html.escape(row.get('remaining_unresolved_bytes', ''))} unresolved</div>
...
<header>
  <div class="wrap">
    ...
    <div class="sub">Applies false-free medium8 selector targets on top of the large32-promoted replay.</div>
  </div>
</header>
<main class="wrap">
  <section class="stats">
    <div class="stat"><div class="label">Added medium8 bytes</div><div class="value ok">{summary['selector_added_bytes']}</div></div>
...
<script>
const TEX_GAP_DECODER_LEN64_PROMOTED_MEDIUM8_REPLAY = {data_json};
</script>
</body>
</html>
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Replay false-free medium8 selector targets over large32-promoted .tex buffers."
    )
    ...
    parser.add_argument("--title", default="Lands of Lore II .tex Len64 Promoted Medium8 Replay")
    ...
    print(f"Added medium8 bytes: {summary['selector_added_bytes']}")
