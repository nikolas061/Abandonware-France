#!/usr/bin/env python3
"""Review exact value-bucket direction rows for reusable context or payloads."""
...
DEFAULT_OUTPUT = Path("output/tex_gap_decoder_len64_promoted_tiny_nonzero_gap_direction_value_exact_context_probe")
...
def build_group_rows(rows: list[dict[str, str]], payload_rows: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    for key, group_rows in grouped.items():
        ...
        if len(group_rows) < 2:
            verdict = "single_exact_context"
        elif repeated_payload_group_rows:
            verdict = "payload_repeat_review"
        elif same_delta:
            verdict = "same_delta_exact_review"
        else:
            verdict = "exact_context_conflict_reject"
        sample = group_rows[0]
        output.append(
            {
...
def build_html(
    ...
    links = " ".join(
        ...
        for label, path in (
            ...
            ("by_exact_context.csv", output_dir / "by_exact_context.csv"),
...
<header>
  <div class="wrap">
    ...
    <div class="sub">Checks exact value-bucket rows for reusable payload or context evidence.</div>
  </div>
</header>
<main class="wrap">
  <section class="stats">
    ...
    <div class="stat"><div class="label">Repeated key bytes</div><div class="value warn">{summary['repeated_key_bytes']}</div></div>
  ...
  <section class="panel"><h2>Exact context groups</h2>{render_table(group_rows, GROUP_FIELDNAMES, 160)}</section>
...
<script>
const TEX_GAP_DECODER_LEN64_PROMOTED_NONZERO_GAP_DIRECTION_VALUE_EXACT_CONTEXT_PROBE = {data_json};
</script>
</body>
</html>
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Probe exact value-bucket direction rows.")
    parser.add_argument("--targets", type=Path, default=DEFAULT_TARGETS)
    parser.add_argument("-o", "--output", type=Path, default=DEFAULT_OUTPUT)
    parser.add_argument(
        "--title",
        default="Lands of Lore II .tex Tiny Nonzero Gap Direction Value Exact Context Probe",
    )
    ...
    write_csv(args.output / "by_exact_context.csv", GROUP_FIELDNAMES, group_rows)
    ...
    print(f"Exact direction/value rows: {summary['target_rows']}")
