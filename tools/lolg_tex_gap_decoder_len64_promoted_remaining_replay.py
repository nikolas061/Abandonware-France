#!/usr/bin/env python3
"""Replay false-free remaining selector targets on top of medium8-promoted buffers."""
...
from lolg_tex_gap_decoder_len64_promoted_large32_replay import (
    FIXTURE_FIELDNAMES,
    SUMMARY_FIELDNAMES,
    bounded_range,
    count_mask,
...
DEFAULT_OUTPUT = Path("output/tex_gap_decoder_len64_promoted_remaining_replay")
DEFAULT_FIXTURES = Path("output/tex_gap_rule_fixtures/manifest.csv")
DEFAULT_FRONTIERS = Path("output/tex_gap_frontier_report/frontiers.csv")
DEFAULT_BASE_FIXTURES = Path("output/tex_gap_decoder_len64_promoted_medium8_replay/fixtures.csv")
DEFAULT_CLEAN_DECISIONS = Path("output/tex_gap_decoder_clean_replay/decisions.csv")
DEFAULT_SELECTOR_TARGETS = [
    Path("output/tex_gap_decoder_len64_promoted_trailing_large32_selector_probe/targets.csv"),
    Path("output/tex_gap_decoder_len64_promoted_leading_len64_selector_probe/targets.csv"),
    Path("output/tex_gap_decoder_len64_promoted_trailing_medium8_selector_probe/targets.csv"),
    Path("output/tex_gap_decoder_len64_promoted_leading_large32_selector_probe/targets.csv"),
    Path("output/tex_gap_decoder_len64_promoted_internal_small_selector_probe/targets.csv"),
]
...
def read_selector_targets(paths: list[Path]) -> list[dict[str, str]]:
    rows: list[dict[str, str]] = []
    for path in paths:
        for row in read_csv(path):
            item = dict(row)
            item["source_report"] = path.parent.as_posix()
            rows.append(item)
    return rows
...
def targets_by_fixture(target_rows: list[dict[str, str]]) -> dict[tuple[str, str, str], list[dict[str, str]]]:
    ...
    for row in target_rows:
        if row.get("greedy_selector") and not row.get("issues"):
    ...
    *,
    ...
    for base_fixture in sorted(base_fixture_rows, key=lambda row: fixture_sort_key(fixture_key(row))):
        ...
        for target in sorted(
            ...
            selector_overlap = overlap_bytes(selector_mask, start, end)
            ...
            if not target.get("signature"):
                target_issues.append("missing_signature")
            if base_overlap:
                target_issues.append("base_known_overlap")
            if selector_overlap:
                target_issues.append("selector_overlap")
            if rejected_overlap:
            ...
            promotion_rows.append(
                {
                    ...
                    "selector_overlap_bytes": str(selector_overlap),
        ...
        decoded_path = fixture_output_dir / f"{stem}_decoded_remaining_promoted.bin"
        ...
        native_preview_path = native_preview_dir / f"{stem}_remaining_promoted.png"
        fullhd_preview_path = fullhd_preview_dir / f"{stem}_remaining_promoted_fullhd.png"
    ...
    summary = {
        ...
        "remaining_unresolved_bytes": str(
            sum(int_value(row, "remaining_unresolved_bytes") for row in output_fixture_rows)
        ),
...
<article class="card">
  ...
  <div class="card-body">
    ...
    <div class="muted">+{html.escape(row.get('selector_added_bytes', ''))} remaining - {html.escape(row.get('remaining_unresolved_bytes', ''))} unresolved</div>
...
<header>
  <div class="wrap">
    ...
    <div class="sub">Applies false-free remaining selector targets on top of the medium8-promoted replay.</div>
  </div>
</header>
<main class="wrap">
  <section class="stats">
    <div class="stat"><div class="label">Added remaining bytes</div><div class="value ok">{summary['selector_added_bytes']}</div></div>
    <div class="stat"><div class="label">Promoted targets</div><div class="value">{summary['promoted_target_rows']}/{summary['target_rows']}</div></div>
...
<script>
const TEX_GAP_DECODER_LEN64_PROMOTED_REMAINING_REPLAY = {data_json};
</script>
</body>
</html>
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Replay false-free remaining selector targets over medium8-promoted .tex buffers."
    )
    ...
    parser.add_argument("--selector-targets", type=Path, nargs="*", default=DEFAULT_SELECTOR_TARGETS)
    parser.add_argument("--title", default="Lands of Lore II .tex Len64 Promoted Remaining Replay")
    ...
    summary, fixture_rows, promotion_rows = build_rows(
        ...
        selector_target_rows=read_selector_targets(args.selector_targets),
    )
    ...
    print(f"Added remaining bytes: {summary['selector_added_bytes']}")
