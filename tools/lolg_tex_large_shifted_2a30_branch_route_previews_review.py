#!/usr/bin/env python3
"""Review routed shifted 0x2a30 branch high-arg2 previews before .tex promotion."""
...
SHEET_SIZE = (1600, 900)
EXPECTED_NATIVE_SIZE = ("48", "128")
EXPECTED_RENDERER_MODE = "cmd20_high_arg2_skip4_markerknown"
DEFAULT_OUTPUT = Path("output/tex_large_shifted_2a30_branch_route_previews_review")
DEFAULT_PREVIEWS_MANIFEST = Path("output/tex_large_shifted_2a30_branch_route_previews/manifest.csv")
...
def write_review_sheet(row: dict[str, str], target: Path, issues: list[str]) -> None:
    ...
    draw_text(draw, (24, 64), f"renderer: {row.get('renderer_mode', '')}", (174, 186, 196))
    draw_text(draw, (24, 86), f"promotion: {row.get('promotion_status', '')}", (174, 186, 196))

    panel_y = 140
    native_panel = (620, 560)
    fullhd_panel = (860, 484)
    panels = (
        ("native", native_path, (24, panel_y), native_panel),
        ("fullhd", fullhd_path, (708, panel_y), fullhd_panel),
    )
    ...
    draw_text(draw, (24, note_y + 46), f"alpha bbox: {row.get('fullhd_alpha_bbox', '')}", (174, 186, 196))
    draw_text(draw, (24, note_y + 68), f"high-arg2 skips: {row.get('high_arg2_skips', '')}", (174, 186, 196))
    draw_text(draw, (24, note_y + 90), f"score: {row.get('renderer_score', '')}", (174, 186, 196))
    draw_text(draw, (24, note_y + 122), f"native: {row.get('native_preview_path', '')}", (137, 151, 162))
    draw_text(draw, (24, note_y + 144), f"fullhd: {row.get('fullhd_preview_path', '')}", (137, 151, 162))
...
def review_manifest_row(output_dir: Path, source: dict[str, str]) -> dict[str, str]:
    ...
    if (source.get("native_width"), source.get("native_height")) != EXPECTED_NATIVE_SIZE:
    ...
    if source.get("renderer_mode") != EXPECTED_RENDERER_MODE:
        issues.append("renderer_mode_mismatch")
    if int_text(source.get("high_arg2_skips")) <= 0:
        issues.append("missing_high_arg2_skips")
    if source.get("final_y") != source.get("renderer_height"):
        issues.append("renderer_final_y_mismatch")
    ...
    row = {
        ...
        "renderer_start": source.get("renderer_start", ""),
        "renderer_width": source.get("renderer_width", ""),
        "renderer_height": source.get("renderer_height", ""),
        "renderer_score": source.get("renderer_score", ""),
        "filled_ratio": source.get("filled_ratio", ""),
        "unique_colors": source.get("unique_colors", ""),
        "high_arg2_skips": source.get("high_arg2_skips", ""),
        "zero_signature_seen": source.get("zero_signature_seen", ""),
        "zero_signature_skipped": source.get("zero_signature_skipped", ""),
        "markerknown_skips": source.get("markerknown_skips", ""),
        "final_x": source.get("final_x", ""),
        "final_y": source.get("final_y", ""),
    ...
    row["promotion_reason"] = "branch_high_arg2_preview_consistent" if not issues else "review_blocked"
    ...
    return [
        {
            ...
            "high_arg2_skips": row.get("high_arg2_skips", ""),
...
def build_summary(
    ...
    clean = len(manifest_rows) == 1 and len(ready_rows) == 1 and len(decision_rows) == 1 and not issue_rows
    if clean:
        verdict = "branch_route_previews_review_ready"
        next_action = "promote routed shifted 0x2a30 branch high-arg2 preview into .tex coverage"
    elif issue_rows:
        verdict = "branch_route_previews_review_blocked"
        next_action = "fix routed shifted 0x2a30 branch high-arg2 preview review issues"
    else:
        verdict = "branch_route_previews_review_incomplete"
        next_action = "complete routed shifted 0x2a30 branch high-arg2 preview review"
    return {
        "scope": "total",
        ...
        "high_arg2_review_rows": str(sum(1 for row in review_rows if int_text(row.get("high_arg2_skips")) > 0)),
        "expected_mode_rows": str(sum(1 for row in review_rows if row.get("renderer_mode") == EXPECTED_RENDERER_MODE)),
...
<article>
  ...
  <div class="body">
    ...
    <div>{html.escape(row.get('renderer_mode', ''))} extra {html.escape(row.get('decoder_extra', ''))}</div>
    <div class="muted">high-arg2 {html.escape(row.get('high_arg2_skips', ''))} / status {html.escape(row.get('promotion_status', ''))}</div>
...
<script>const TEX_LARGE_SHIFTED_2A30_BRANCH_ROUTE_PREVIEWS_REVIEW = {data_json};</script>
</body>
</html>
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Review routed shifted 0x2a30 branch high-arg2 previews.")
    parser.add_argument("-o", "--output", type=Path, default=DEFAULT_OUTPUT)
    parser.add_argument("--previews-manifest", type=Path, default=DEFAULT_PREVIEWS_MANIFEST)
    parser.add_argument(
        "--title",
        default="Lands of Lore II .tex Large Shifted 2a30 Branch High-Arg2 Previews Review",
    )
    ...
    print(f"Ready rows: {summary['review_ready_rows']}")
