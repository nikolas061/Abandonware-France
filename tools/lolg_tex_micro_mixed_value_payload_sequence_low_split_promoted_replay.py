#!/usr/bin/env python3
"""Promote guarded mixed-value sequence low-split candidates into fixture buffers."""
...
from lolg_tex_micro_mixed_value_payload_prefix_sequence_promoted_replay import (
    ...
    DEFAULT_FIXTURES,
    DEFAULT_FRONTIERS,
    DEFAULT_OUTPUT as DEFAULT_PREFIX_SEQUENCE_PROMOTED_REPLAY_OUTPUT,
    archive_key,
    byte_hex,
    parse_hex_byte,
)
from lolg_tex_micro_mixed_value_payload_sequence_low_split_probe import (
    DEFAULT_OUTPUT as DEFAULT_LOW_SPLIT_OUTPUT,
)


DEFAULT_OUTPUT = Path("output/tex_micro_mixed_value_payload_sequence_low_split_promoted_replay")
DEFAULT_BASE_FIXTURES = DEFAULT_PREFIX_SEQUENCE_PROMOTED_REPLAY_OUTPUT / "fixtures.csv"
DEFAULT_LOW_SPLIT_SLOTS = DEFAULT_LOW_SPLIT_OUTPUT / "slots.csv"
...
SUMMARY_FIELDNAMES = [
    ...
    "promoted_rows",
    "base_clean_bytes",
    "low_split_added_bytes",
    "low_split_exact_bytes",
    "low_split_false_bytes",
    "skipped_known_bytes",
...
def grouped_candidate_slots(rows: list[dict[str, str]]) -> dict[tuple[str, str, str], list[dict[str, str]]]:
    ...
    for row in rows:
        if row.get("state") != "replayable_unknown":
            continue
        if not row.get("best_split_predicted_byte"):
            continue
...
def build_rows(
    *,
    ...
    low_split_slot_rows: list[dict[str, str]],
    ...
    slot_groups = grouped_candidate_slots(low_split_slot_rows)
    ...
    for base_fixture in sorted(
        ...
        low_split_mask = bytearray(len(expected))
        stats: Counter[str] = Counter()
        for slot in slot_groups.get(key, []):
            target_issues: list[str] = []
            offset = int_value(slot, "absolute_offset")
            predicted = parse_hex_byte(slot.get("best_split_predicted_byte", ""), target_issues, "predicted_byte")
            ...
            low_split_overlap = overlap_bytes(low_split_mask, offset, offset + 1)
            rejected_overlap = overlap_bytes(rejected_mask, offset, offset + 1)
            known_prerequisites = [part for part in slot.get("known_prerequisites", "").split("|") if part]

            if offset < 0 or offset >= len(expected):
                target_issues.append("offset_out_of_range")
            if slot.get("best_split_verdict") not in {"correct", ""}:
                target_issues.append("split_probe_marked_non_correct")
            if known_prerequisites and not all(value == "1" for value in known_prerequisites):
            ...
            if predicted is not None and expected_value is not None and predicted != expected_value:
                target_issues.append("low_split_would_write_false_byte")
            ...
            if low_split_overlap:
                target_issues.append("low_split_overlap")

            false_bytes = 1 if "low_split_would_write_false_byte" in target_issues else 0
            added_bytes = 0
            exact_bytes = 0
            skipped_known = 0
            skipped_rejected = 0
            if not target_issues:
                if base_overlap:
                    ...
                    low_split_mask[offset] = 0xFF
                    added_bytes = 1
                    exact_bytes = 1
            else:
                issue_rows += 1

            stats["low_split_candidate_rows"] += 1
            stats["low_split_added_bytes"] += added_bytes
            stats["low_split_exact_bytes"] += exact_bytes
            stats["low_split_false_bytes"] += false_bytes
            ...
            promotion_rows.append(
                {
                    ...
                    "predicted_byte": slot.get("best_split_predicted_byte", ""),
                    "feature_set": slot.get("best_split_feature_set", ""),
                    "feature_context": slot.get("best_split_context", ""),
                    "base_known_overlap_bytes": str(base_overlap),
                    "low_split_overlap_bytes": str(low_split_overlap),
                    "rejected_overlap_bytes": str(rejected_overlap),
                    "prerequisite_offsets": slot.get("prerequisite_offsets", ""),
                    "known_prerequisites": slot.get("known_prerequisites", ""),
                    "low_split_added_bytes": str(added_bytes),
                    "low_split_exact_bytes": str(exact_bytes),
                    "low_split_false_bytes": str(false_bytes),
        ...
        decoded_path = fixture_output_dir / f"{stem}_decoded_sequence_low_split.bin"
        known_mask_path = fixture_output_dir / f"{stem}_known_mask.bin"
        low_split_mask_path = fixture_output_dir / f"{stem}_sequence_low_split_mask.bin"
        decoded_path.write_bytes(decoded)
        known_mask_path.write_bytes(known_mask)
        low_split_mask_path.write_bytes(low_split_mask)
        native_preview_path = native_preview_dir / f"{stem}_sequence_low_split.png"
        fullhd_preview_path = fullhd_preview_dir / f"{stem}_sequence_low_split_fullhd.png"
        fullhd_width, fullhd_height = render_preview(
            expected=expected,
            decoded=bytes(decoded),
            known_mask=bytes(known_mask),
            risk_mask=bytes(low_split_mask),
        ...
        output_fixture_rows.append(
            {
                ...
                "low_split_candidate_rows": str(stats["low_split_candidate_rows"]),
                "low_split_added_bytes": str(stats["low_split_added_bytes"]),
                "low_split_exact_bytes": str(stats["low_split_exact_bytes"]),
                "low_split_false_bytes": str(stats["low_split_false_bytes"]),
                ...
                "low_split_mask_path": low_split_mask_path.as_posix(),
    ...
    summary = {
        ...
        "split_candidate_rows": str(len(promotion_rows)),
        "promoted_rows": str(sum(1 for row in promotion_rows if int_value(row, "low_split_added_bytes"))),
        "base_clean_bytes": str(sum(int_value(row, "base_clean_bytes") for row in output_fixture_rows)),
        "low_split_added_bytes": str(sum(int_value(row, "low_split_added_bytes") for row in output_fixture_rows)),
        "low_split_exact_bytes": str(sum(int_value(row, "low_split_exact_bytes") for row in output_fixture_rows)),
        "low_split_false_bytes": str(sum(int_value(row, "low_split_false_bytes") for row in output_fixture_rows)),
...
<div class="grid">
  <div class="box"><div class="num">{summary['low_split_added_bytes']}</div><div class="muted">low-split added bytes</div></div>
  <div class="box"><div class="num">{summary['promoted_rows']}/{summary['split_candidate_rows']}</div><div class="muted">promoted rows</div></div>
  <div class="box"><div class="num">{summary['low_split_false_bytes']}</div><div class="muted">false bytes</div></div>
...
<script type="application/json" id="mixed-value-sequence-low-split-promoted-data">{html.escape(data_json)}</script>
</body>
</html>
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Promote guarded sequence low-split candidates.")
    ...
    parser.add_argument("--low-split-slots", type=Path, default=DEFAULT_LOW_SPLIT_SLOTS)
    parser.add_argument("--title", default="Lands of Lore II .tex Sequence Low Split Promoted Replay")
    ...
    summary, fixture_rows, promotion_rows = build_rows(
        ...
        low_split_slot_rows=read_csv(args.low_split_slots),
    )
    ...
    print(f"Promoted rows: {summary['promoted_rows']}/{summary['split_candidate_rows']}")
    print(f"Added low-split bytes: {summary['low_split_added_bytes']}")
    print(f"False bytes: {summary['low_split_false_bytes']}")
