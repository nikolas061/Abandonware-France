#!/usr/bin/env python3
"""Probe minority low exceptions after coarse bucket split."""
...
from lolg_tex_gradient_sequence_high_safe_low_bucket_split_probe import (
    BUCKET_ORDER,
    SLOT_FIELDNAMES as BUCKET_SPLIT_SLOT_FIELDNAMES,
    context_functions,
    low_bucket,
    strict_prediction,
)
...
DEFAULT_SLOTS = Path("output/tex_gradient_sequence_high_safe_low_bucket_split/slots.csv")
DEFAULT_OUTPUT = Path("output/tex_gradient_sequence_high_safe_low_exception")
...
def target_key(bucket: str, target_low: str) -> str:
    return f"{bucket}:{target_low}"
...
def majority_by_bucket(entries_by_bucket: dict[str, list[dict[str, object]]]) -> dict[str, str]:
    output: dict[str, str] = {}
    for bucket in BUCKET_ORDER:
        values = Counter(str(entry["low"]) for entry in entries_by_bucket.get(bucket, []))
        output[bucket] = values.most_common(1)[0][0] if values else ""
    return output


def exception_targets(
    entries_by_bucket: dict[str, list[dict[str, object]]],
    majority: dict[str, str],
) -> list[tuple[str, str]]:
    targets: list[tuple[str, str]] = []
    for bucket in BUCKET_ORDER:
        counts = Counter(str(entry["low"]) for entry in entries_by_bucket.get(bucket, []))
        for value, count in counts.most_common():
            if value != majority.get(bucket, "") and count > 0:
                targets.append((bucket, value))
    return targets


def evaluate_exception_candidate(
    entries: list[dict[str, object]],
    bucket: str,
    majority_low: str,
    target_low: str,
    ...
    target_slots = sum(1 for entry in entries if str(entry["low"]) == target_low)
    for entry in entries:
        context = context_func(entry)
        low = str(entry["low"])
        all_counts[context][low] += 1
        row_counts[(int(entry["row_index"]), context)][low] += 1
        grouped[context].append(entry)

    pure_target_context_slots = 0
    ...
    for context, group in grouped.items():
        ...
        if prediction == target_low:
            pure_target_context_slots += len(group)
    ...
    if predicted == 0:
        verdict = "no_exception_signal"
    elif false == 0 and correct >= 8:
        verdict = "false_free_exception_review"
    elif false == 0:
        verdict = "sparse_false_free_exception"
    elif correct > false:
        verdict = "partial_exception_hint"
    else:
        verdict = "conflicted_exception_context"

    return {
        "rank": 0,
        "target_key": target_key(bucket, target_low),
        "bucket": bucket,
        "majority_low": majority_low,
        "target_low": target_low,
        "context_family": context_family,
        "contexts": len(grouped),
        "target_slots": target_slots,
        "pure_target_context_slots": pure_target_context_slots,
        ...
        "target_recall": ratio(correct, target_slots),
        "predicted_values": "|".join(f"{value}:{count}" for value, count in predicted_values.most_common(8)),
        "verdict": verdict,
        "sample_context": sample_context,
        "sample_prediction": target_low if sample_context else "",
    }
...
def annotate_rows(
    ...
    for target in target_rows:
        ...
        predictions = exception_predictions(entries_by_bucket.get(bucket, []), target_low, context)
        ...
        annotated.update(
            {
                ...
                "exception_prediction": prediction_text,
...
def build_summary(
    ...
    return {
        "scope": "total",
        "candidate_mode": "minority_low_exception_loo",
        "slots": len(rows),
...
<script type="application/json" id="gradient-sequence-high-safe-low-exception-data">{html.escape(data_json)}</script>
</body>
</html>
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Probe minority low exceptions after bucket split.")
    parser.add_argument("--slots", type=Path, default=DEFAULT_SLOTS)
    parser.add_argument("-o", "--output", type=Path, default=DEFAULT_OUTPUT)
    parser.add_argument(
        "--title",
        default="Lands of Lore II .tex Gradient Sequence High-Safe Low Exception Probe",
    )
