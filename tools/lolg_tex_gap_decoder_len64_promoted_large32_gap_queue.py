#!/usr/bin/env python3
"""Rank remaining .tex gap spans after the promoted large32 replay."""
...
DEFAULT_OUTPUT = Path("output/tex_gap_decoder_len64_promoted_large32_gap_queue")
DEFAULT_FIXTURE_MANIFEST = Path("output/tex_gap_rule_fixtures/manifest.csv")
DEFAULT_PROMOTED_FIXTURES = Path("output/tex_gap_decoder_len64_promoted_large32_replay/fixtures.csv")
...
<header>
  <div class="wrap">
    ...
    <div class="sub">Remaining spans after len64 plus large32 promoted replay.</div>
  </div>
</header>
<main class="wrap">
  <section class="stats">
    <div class="stat"><div class="label">Clean bytes</div><div class="value ok">{html.escape(summary['clean_bytes'])}</div></div>
    <div class="stat"><div class="label">Large32 added</div><div class="value ok">{html.escape(summary['selector_added_bytes'])}</div></div>
...
<script>
const TEX_GAP_DECODER_LEN64_PROMOTED_LARGE32_GAP_QUEUE = {data_json};
</script>
</body>
</html>
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Rank .tex gaps after the promoted large32 replay.")
    ...
    parser.add_argument("--title", default="Lands of Lore II .tex Len64 Promoted Large32 Gap Queue")
    ...
    print(f"Large32 added bytes: {summary['selector_added_bytes']}")
