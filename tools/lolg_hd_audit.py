...
DEFAULT_TEX_LARGE_SHIFTED_2A30_FIELD16_DECODER_PROMOTED_PACK_MANIFEST = Path(
    "output/tex_large_shifted_2a30_field16_decoder_promoted_pack/manifest.csv"
)
DEFAULT_TEX_LARGE_SHIFTED_2A30_FIELD16_DECODER_PROMOTED_PACK_HTML = Path(
...
DEFAULT_TEX_AUGMENTED_FIELD16_DECODER = Path("output/tex_augmented_coverage/field16_decoder_promotions.csv")
...
DEFAULT_TEX_LARGE_SHIFTED_2A30_FIELD16_SMALL_DELTA_GUARD_REVIEW_TARGETS = Path(
    "output/tex_large_shifted_2a30_field16_small_delta_guard_review/targets.csv"
)
DEFAULT_TEX_LARGE_SHIFTED_2A30_FIELD16_SMALL_DELTA_GUARD_REVIEW_SUPPORT = Path(
    "output/tex_large_shifted_2a30_field16_small_delta_guard_review/support.csv"
)
DEFAULT_TEX_LARGE_SHIFTED_2A30_FIELD16_SMALL_DELTA_GUARD_REVIEW_GUARDS = Path(
    "output/tex_large_shifted_2a30_field16_small_delta_guard_review/guards.csv"
)
DEFAULT_TEX_LARGE_SHIFTED_2A30_FIELD16_SMALL_DELTA_GUARD_REVIEW_DECISIONS = Path(
    "output/tex_large_shifted_2a30_field16_small_delta_guard_review/decisions_template.tsv"
)
...
DEFAULT_TEX_LARGE_SHIFTED_2A30_FIELD16_DECODER_PREVIEWS_MANIFEST = Path(
    "output/tex_large_shifted_2a30_field16_decoder_previews/manifest.csv"
)
...
SUMMARY_FIELDNAMES = [
    ...
    "tex_large_shifted_2a30_field16_decoder_promoted_pack_eligible",
    ...
    "tex_augmented_field16_decoder_unique_pcx",
    "tex_augmented_exact_alias_decoded_raw_or_field16_unique_pcx",
...
def audit_tex_augmented_coverage(
    ...
    field16_decoder_promotions: Path,
    html_report: Path,
) -> tuple[dict[str, str], int, int, int, int, int, int]:
    if not summary.exists():
        return missing_gate("tex_augmented_coverage", summary), 0, 0, 0, 0, 0, 0
    if not references.exists():
        return missing_gate("tex_augmented_coverage", references), 0, 0, 0, 0, 0, 0
    if not aliases.exists():
        return missing_gate("tex_augmented_coverage", aliases), 0, 0, 0, 0, 0, 0
    if not material_decodes.exists():
        return missing_gate("tex_augmented_coverage", material_decodes), 0, 0, 0, 0, 0, 0
    if not raw_same_archive_promotions.exists():
        return missing_gate("tex_augmented_coverage", raw_same_archive_promotions), 0, 0, 0, 0, 0, 0
    if not field16_decoder_promotions.exists():
        return missing_gate("tex_augmented_coverage", field16_decoder_promotions), 0, 0, 0, 0, 0, 0
    if not html_report.exists():
        return missing_gate("tex_augmented_coverage", html_report), 0, 0, 0, 0, 0, 0
    ...
    field16_rows = read_csv(field16_decoder_promotions)
    text = html_report.read_text(errors="replace")
    issues: list[str] = []
    if len(summary_rows) != 1:
        issues.append("summary_row_count_invalid")
    ...
    field16_reference_rows = int_value(total, "field16_decoder_reference_rows")
    field16_unique = int_value(total, "field16_decoder_unique_pcx")
    field16_assets = int_value(total, "field16_decoder_assets")
    ...
    exact_alias_decoded_raw_or_field16 = int_value(total, "exact_alias_decoded_raw_or_field16_unique_pcx")
    ...
    field16_names = {
        row.get("normalized_pcx_name", "")
        for row in reference_rows
        if row.get("coverage_status") == "field16_decoder"
    }
    unresolved_names = {
        row.get("normalized_pcx_name", "")
        for row in reference_rows
    ...
    eligible_field16 = [row for row in field16_rows if row.get("coverage_eligible") == "yes"]
    ...
    if field16_reference_rows != sum(1 for row in reference_rows if row.get("coverage_status") == "field16_decoder"):
        issues.append("tex_augmented_field16_reference_count_mismatch")
    if field16_unique != len(field16_names):
        issues.append("tex_augmented_field16_unique_count_mismatch")
    if field16_assets != len(eligible_field16):
        issues.append("tex_augmented_field16_asset_count_mismatch")
    ...
    if exact_alias_decoded_raw_or_field16 != len(
        exact_names | alias_names | decoded_names | raw_names | field16_names
    ):
        issues.append("tex_augmented_exact_alias_decoded_raw_or_field16_count_mismatch")
    ...
    missing_field16_paths = 0
    for row in eligible_field16:
        value = row.get("promoted_fullhd_path", "")
        if not value or not Path(value).exists():
            missing_field16_paths += 1
        if row.get("issues"):
            issues.append("field16_decoder_row_has_issues")
    if missing_field16_paths:
        issues.append(f"missing_field16_decoder_paths:{missing_field16_paths}")
    ...
    return (
        gate(
            ...
            ok,
            expected=".tex exact, alias, material decode, raw and field16 decoder coverage report is internally consistent",
            actual=(
                f"unique={unique_likely}, exact={exact_unique}, alias={alias_unique}, "
                f"decoded={decoded_unique}, raw={raw_unique}, field16={field16_unique}, "
                f"covered_augmented={exact_alias_decoded_raw_or_field16}, "
        ...
        field16_unique if ok else 0,
        exact_alias_decoded_raw_or_field16 if ok else 0,
...
def audit_tex_large_rejected_decoder_profile(
    ...
    control_path_groups = int_value(total, "control_path_groups")
    dominant_control_path = total.get("dominant_control_path", "")
    dominant_control_path_rows = int_value(total, "dominant_control_path_rows")
    ...
    if control_path_groups != len(control_rows) or control_path_groups != len(control_path_counts):
        issues.append("large_rejected_profile_control_path_count_mismatch")
    ...
    if control_path_counts:
        top_path, top_path_rows = control_path_counts.most_common(1)[0]
        if dominant_control_path != top_path or dominant_control_path_rows != top_path_rows:
            issues.append("large_rejected_profile_dominant_control_path_mismatch")
    ...
    if shifted_2a30_rows != control_path_counts["shifted_2a30_header"]:
        issues.append("large_rejected_profile_shifted_2a30_count_mismatch")
    if shared_2700302b_rows != control_path_counts["shared_2700302b_header"]:
    ...
    for row in control_rows:
        ...
        if int_value(row, "rows") != control_path_counts[control_path]:
            issues.append(f"large_rejected_profile_control_path_row_mismatch:{control_path}")
...
def audit_tex_large_shifted_2a30_field16_small_delta_guard_review(
    summary: Path,
    targets_path: Path,
    support_path: Path,
    guards_path: Path,
    ...
    if not summary.exists():
        return missing_gate("tex_large_shifted_2a30_field16_small_delta_guard_review", summary), 0, 0, 0
    if not targets_path.exists():
        return missing_gate("tex_large_shifted_2a30_field16_small_delta_guard_review", targets_path), 0, 0, 0
    if not support_path.exists():
        return missing_gate("tex_large_shifted_2a30_field16_small_delta_guard_review", support_path), 0, 0, 0
    if not guards_path.exists():
        return missing_gate("tex_large_shifted_2a30_field16_small_delta_guard_review", guards_path), 0, 0, 0
    if not decisions_path.exists():
        return missing_gate("tex_large_shifted_2a30_field16_small_delta_guard_review", decisions_path), 0, 0, 0
    if not html_report.exists():
        return missing_gate("tex_large_shifted_2a30_field16_small_delta_guard_review", html_report), 0, 0, 0
    ...
    false_positive_rows = int_value(total, "false_positive_rows")
    large_rows = int_value(total, "large_rows")
    large_target_rows = int_value(total, "large_target_rows")
    large_target_exact_rows = int_value(total, "large_target_exact_rows")
    remaining_large_rows = int_value(total, "remaining_large_rows")
    remaining_target_rows = int_value(total, "remaining_target_rows")
    remaining_target_exact_rows = int_value(total, "remaining_target_exact_rows")
    support_count = int_value(total, "support_rows")
    skipped_context_rows = int_value(total, "skipped_context_rows")
    positive_target_rows = int_value(total, "positive_target_rows")
    negative_target_rows = int_value(total, "negative_target_rows")
    zero_target_rows = int_value(total, "zero_target_rows")
    selected_guard_rows = int_value(total, "selected_guard_rows")
    guard_rows = int_value(total, "guard_rows")
    decision_template_rows = int_value(total, "decision_template_rows")
    issue_rows = int_value(total, "issue_rows")

    if candidate_rows != 10:
        issues.append("field16_small_delta_review_candidate_count_mismatch")
    if applicable_rows != 6 or exact_rows != 6:
        issues.append("field16_small_delta_review_applicable_exact_mismatch")
    if false_positive_rows != 0:
        issues.append(f"false_positive_rows:{false_positive_rows}")
    if large_rows != 4:
        issues.append("field16_small_delta_review_large_count_mismatch")
    if large_target_rows != len(target_rows):
        issues.append("field16_small_delta_review_target_summary_count_mismatch")
    if large_target_rows != 4 or large_target_exact_rows != 4:
        issues.append("field16_small_delta_review_target_coverage_mismatch")
    if remaining_large_rows != 2 or remaining_target_rows != 2 or remaining_target_exact_rows != 2:
        issues.append("field16_small_delta_review_remaining_coverage_mismatch")
    if support_count != sum(1 for row in support_rows if row.get("support_role") == "nonlarge_guard_support"):
        issues.append("field16_small_delta_review_support_count_mismatch")
    if support_count != 2:
        issues.append("field16_small_delta_review_support_expected_count_mismatch")
    if skipped_context_rows != sum(
        1 for row in support_rows if row.get("support_role") == "skipped_large_negative_context"
    ):
        issues.append("field16_small_delta_review_context_count_mismatch")
    if skipped_context_rows != 4:
        issues.append("field16_small_delta_review_context_expected_count_mismatch")
    if positive_target_rows != 1 or negative_target_rows != 1 or zero_target_rows != 2:
        issues.append("field16_small_delta_review_target_group_count_mismatch")
    if selected_guard_rows != 3:
        issues.append("field16_small_delta_review_selected_guard_count_mismatch")
    if guard_rows != len(guard_rows_data) or guard_rows != 4:
        issues.append("field16_small_delta_review_guard_count_mismatch")
    if decision_template_rows != len(decision_rows) or decision_template_rows != 4:
        issues.append("field16_small_delta_review_decision_count_mismatch")
    if issue_rows:
        issues.append(f"issue_rows:{issue_rows}")

    target_by_name = {row.get("pcx_name", ""): row for row in target_rows}
    expected_targets = {
        "barsgld.pcx": ("L14_HT", "positive_small_nextlow_minus2", "24", "yes"),
        "dragend.pcx": ("L3_DH", "negative_small_extra36", "36", "yes"),
        "stump2.pcx": ("L12_CM", "zero_delta_extra40", "40", "no"),
        "catbas.pcx": ("L5_HC", "zero_delta_extra40", "40", "no"),
    }
    if set(target_by_name) != set(expected_targets):
        issues.append("field16_small_delta_review_targets_unexpected")
    else:
        for pcx_name, (archive_tag, guard_id, predicted_extra, remaining_large) in expected_targets.items():
            row = target_by_name[pcx_name]
            if row.get("archive_tag") != archive_tag:
                issues.append(f"field16_small_delta_review_{pcx_name}_archive_mismatch")
            if row.get("guard_id") != guard_id:
                issues.append(f"field16_small_delta_review_{pcx_name}_guard_mismatch")
            if row.get("predicted_extra") != predicted_extra or row.get("oracle_extra") != predicted_extra:
                issues.append(f"field16_small_delta_review_{pcx_name}_extra_mismatch")
            if row.get("remaining_large") != remaining_large:
                issues.append(f"field16_small_delta_review_{pcx_name}_remaining_mismatch")
            if row.get("exact_match") != "yes" or row.get("promotion_status") != "ready" or row.get("issues"):
                issues.append(f"field16_small_delta_review_{pcx_name}_not_ready")

    support_by_name = {row.get("pcx_name", ""): row for row in support_rows}
    expected_support = {
        "6.pcx": ("nonlarge_guard_support", "positive_small_nextlow_minus2", "32"),
        "pillar.pcx": ("nonlarge_guard_support", "negative_small_extra36", "36"),
        "ggbridge2.pcx": ("skipped_large_negative_context", "skip_large_negative_context", ""),
        "colapse.pcx": ("skipped_large_negative_context", "skip_large_negative_context", ""),
        "hrglass.pcx": ("skipped_large_negative_context", "skip_large_negative_context", ""),
        "lantern.pcx": ("skipped_large_negative_context", "skip_large_negative_context", ""),
    }
    if set(support_by_name) != set(expected_support):
        issues.append("field16_small_delta_review_support_unexpected")
    else:
        for pcx_name, (support_role, guard_id, predicted_extra) in expected_support.items():
            row = support_by_name[pcx_name]
            if row.get("support_role") != support_role:
                issues.append(f"field16_small_delta_review_{pcx_name}_support_role_mismatch")
            if row.get("guard_id") != guard_id:
                issues.append(f"field16_small_delta_review_{pcx_name}_support_guard_mismatch")
            if row.get("predicted_extra") != predicted_extra:
                issues.append(f"field16_small_delta_review_{pcx_name}_support_prediction_mismatch")
            if row.get("issues"):
                issues.append(f"field16_small_delta_review_{pcx_name}_support_issue")

    guard_by_id = {row.get("guard_id", ""): row for row in guard_rows_data}
    expected_guard_rows = {
        "positive_small_nextlow_minus2": ("2", "1", "1", "1", "0", "promote_large_exact_targets", "ready"),
        "negative_small_extra36": ("2", "1", "1", "1", "0", "promote_large_exact_targets", "ready"),
        "zero_delta_extra40": ("2", "2", "2", "0", "0", "promote_large_exact_targets", "ready"),
        "skip_large_negative_context": ("4", "0", "0", "4", "4", "context_only", "not_promoted_context_skipped"),
    }
    if set(guard_by_id) != set(expected_guard_rows):
        issues.append("field16_small_delta_review_guard_ids_mismatch")
    else:
        for guard_id, expected in expected_guard_rows.items():
            row = guard_by_id[guard_id]
            actual = (
            ...
            if actual != expected:
                issues.append(f"field16_small_delta_review_{guard_id}_counts_mismatch")
            if row.get("false_positive_rows") != "0":
                issues.append(f"field16_small_delta_review_{guard_id}_false_positive")

    decision_by_id = {row.get("promotion_id", ""): row for row in decision_rows}
    target_ids = {row.get("promotion_id", "") for row in target_rows}
    if set(decision_by_id) != target_ids:
        issues.append("field16_small_delta_review_decision_ids_mismatch")
    if any(row.get("review_status") != "ready" for row in decision_rows):
        issues.append("field16_small_delta_review_decision_status_mismatch")
    if total.get("review_verdict") != "small_delta_guard_review_ready":
        issues.append("field16_small_delta_review_verdict_mismatch")
    if "const TEX_LARGE_SHIFTED_2A30_FIELD16_SMALL_DELTA_GUARD_REVIEW = " not in text:
        issues.append("missing_tex_large_shifted_2a30_field16_small_delta_guard_review_json")
    ...
    return (
        gate(
            ...
            ok,
            expected="guarded shifted 0x2a30 field16 small-delta review selects exactly 4 ready large targets",
            actual=(
                f"targets={large_target_exact_rows}/{large_rows}, remaining={remaining_target_exact_rows}/"
                f"{remaining_large_rows}, guards={selected_guard_rows}, issues={issue_rows}"
            ),
            evidence=f"{summary};{html_report}",
            issues=issues,
        ),
...
def audit_tex_large_shifted_2a30_field16_decoder_previews(
    summary: Path,
    manifest_path: Path,
    html_report: Path,
) -> tuple[dict[str, str], int, int, int]:
    if not summary.exists():
        return missing_gate("tex_large_shifted_2a30_field16_decoder_previews", summary), 0, 0, 0
    if not manifest_path.exists():
        return missing_gate("tex_large_shifted_2a30_field16_decoder_previews", manifest_path), 0, 0, 0
    if not html_report.exists():
        return missing_gate("tex_large_shifted_2a30_field16_decoder_previews", html_report), 0, 0, 0
    ...
    route_rows = int_value(total, "route_rows")
    routed_rows = int_value(total, "routed_rows")
    manifest_count = int_value(total, "manifest_rows")
    native_previews = int_value(total, "native_previews")
    fullhd_previews = int_value(total, "fullhd_previews")
    source_previews = int_value(total, "source_previews")
    missing_sources = int_value(total, "missing_source_previews")
    issue_rows = int_value(total, "issue_rows")

    native_rows = [row for row in manifest_rows if row.get("native_preview_exists") == "yes"]
    fullhd_rows = [
        row
        for row in manifest_rows
        if row.get("fullhd_preview_exists") == "yes"
        and (row.get("fullhd_width"), row.get("fullhd_height")) == (str(TARGET_SIZE[0]), str(TARGET_SIZE[1]))
    ]
    source_rows = [row for row in manifest_rows if row.get("source_preview_exists") == "yes"]
    manifest_issue_rows = [row for row in manifest_rows if row.get("issues")]

    if route_rows != 9:
        issues.append("field16_decoder_preview_route_count_mismatch")
    if routed_rows != 4:
        issues.append("field16_decoder_preview_routed_count_mismatch")
    if manifest_count != len(manifest_rows) or manifest_count != 4:
        issues.append("field16_decoder_preview_manifest_count_mismatch")
    if native_previews != len(native_rows) or native_previews != 4:
        issues.append("field16_decoder_preview_native_count_mismatch")
    if fullhd_previews != len(fullhd_rows) or fullhd_previews != 4:
        issues.append("field16_decoder_preview_fullhd_count_mismatch")
    if source_previews != len(source_rows) or source_previews != 4:
        issues.append("field16_decoder_preview_source_count_mismatch")
    if missing_sources != 0:
        issues.append("field16_decoder_preview_missing_sources")
    if issue_rows != len(manifest_issue_rows) or issue_rows != 0:
        issues.append(f"issue_rows:{issue_rows}")
    ...
    rows_by_name = {row.get("pcx_name", ""): row for row in manifest_rows}
    if set(rows_by_name) != set(expected_rows):
        issues.append("field16_decoder_preview_rows_unexpected")
    else:
        for pcx_name, expected in expected_rows.items():
            row = rows_by_name[pcx_name]
            actual = (row.get("archive_tag"), row.get("decoder_rule"), row.get("decoder_extra"))
            if actual != expected:
                issues.append(f"field16_decoder_preview_{pcx_name}_row_mismatch")
            if row.get("native_width") != "48" or row.get("native_height") != "128":
                issues.append(f"field16_decoder_preview_{pcx_name}_native_size_mismatch")
            if (row.get("fullhd_width"), row.get("fullhd_height")) != (str(TARGET_SIZE[0]), str(TARGET_SIZE[1])):
                issues.append(f"field16_decoder_preview_{pcx_name}_fullhd_size_mismatch")
            for field in ("source_preview_path", "native_preview_path", "fullhd_preview_path"):
                if not row.get(field) or not Path(row[field]).exists():
                    issues.append(f"field16_decoder_preview_{pcx_name}_{field}_missing")
            if row.get("issues"):
                issues.append(f"field16_decoder_preview_{pcx_name}_issue")

    if total.get("review_verdict") != "field16_decoder_previews_ready":
        issues.append("field16_decoder_preview_verdict_mismatch")
    if "const TEX_LARGE_SHIFTED_2A30_FIELD16_DECODER_PREVIEWS = " not in text:
        issues.append("missing_tex_large_shifted_2a30_field16_decoder_previews_json")
    ...
    return (
        gate(
            ...
            ok,
            expected="materialized 4 routed shifted 0x2a30 field16 decoder previews as native and Full HD PNGs",
            actual=(
                f"native={native_previews}, fullhd={fullhd_previews}, "
                f"sources={source_previews}, issues={issue_rows}"
            ),
            evidence=f"{summary};{html_report}",
            issues=issues,
        ),
...
def audit_tex_large_shifted_2a30_field16_decoder_promoted_pack(
    summary: Path,
    manifest_path: Path,
    html_report: Path,
) -> tuple[dict[str, str], int]:
    if not summary.exists():
        return missing_gate("tex_large_shifted_2a30_field16_decoder_promoted_pack", summary), 0
    if not manifest_path.exists():
        return missing_gate("tex_large_shifted_2a30_field16_decoder_promoted_pack", manifest_path), 0
    if not html_report.exists():
        return missing_gate("tex_large_shifted_2a30_field16_decoder_promoted_pack", html_report), 0
    ...
    review_rows = int_value(total, "review_rows")
    ready_rows = int_value(total, "ready_rows")
    eligible_rows = int_value(total, "coverage_eligible_rows")
    eligible_unique = int_value(total, "coverage_eligible_unique_pcx")
    native_assets = int_value(total, "native_assets")
    fullhd_assets = int_value(total, "fullhd_assets")
    missing_native = int_value(total, "missing_native_paths")
    missing_fullhd = int_value(total, "missing_fullhd_paths")
    ...
    eligible_manifest_rows = [row for row in manifest_rows if row.get("coverage_eligible") == "yes"]
    native_rows = [row for row in manifest_rows if row.get("promoted_native_exists") == "yes"]
    fullhd_rows = [
        row
        for row in manifest_rows
        if row.get("promoted_fullhd_exists") == "yes"
        and (row.get("promoted_fullhd_width"), row.get("promoted_fullhd_height"))
        == (str(TARGET_SIZE[0]), str(TARGET_SIZE[1]))
    ]
    manifest_issue_rows = [row for row in manifest_rows if row.get("issues")]

    if review_rows != 4 or review_rows != len(manifest_rows):
        issues.append("field16_decoder_promoted_pack_review_count_mismatch")
    if ready_rows != 4:
        issues.append("field16_decoder_promoted_pack_ready_count_mismatch")
    if eligible_rows != len(eligible_manifest_rows) or eligible_rows != 4:
        issues.append("field16_decoder_promoted_pack_eligible_count_mismatch")
    if eligible_unique != 4:
        issues.append("field16_decoder_promoted_pack_unique_count_mismatch")
    if native_assets != len(native_rows) or native_assets != 4:
        issues.append("field16_decoder_promoted_pack_native_count_mismatch")
    if fullhd_assets != len(fullhd_rows) or fullhd_assets != 4:
        issues.append("field16_decoder_promoted_pack_fullhd_count_mismatch")
    if missing_native or missing_fullhd:
        issues.append("field16_decoder_promoted_pack_missing_assets")
    if issue_rows != len(manifest_issue_rows) or issue_rows != 0:
        issues.append(f"issue_rows:{issue_rows}")
    ...
    rows_by_name = {row.get("pcx_name", ""): row for row in manifest_rows}
    if set(rows_by_name) != set(expected_rows):
        issues.append("field16_decoder_promoted_pack_rows_unexpected")
    for pcx_name, expected in expected_rows.items():
        row = rows_by_name.get(pcx_name)
        if not row:
            continue
        actual = (row.get("archive_tag"), row.get("decoder_rule"), row.get("decoder_extra"))
        if actual != expected:
            issues.append(f"field16_decoder_promoted_pack_{pcx_name}_row_mismatch")
        if row.get("review_status") != "ready" or row.get("coverage_eligible") != "yes":
            issues.append(f"field16_decoder_promoted_pack_{pcx_name}_status_mismatch")
        for field in ("source_native_path", "source_fullhd_path", "promoted_native_path", "promoted_fullhd_path"):
            if not row.get(field) or not Path(row[field]).exists():
                issues.append(f"field16_decoder_promoted_pack_{pcx_name}_{field}_missing")
        if (row.get("promoted_fullhd_width"), row.get("promoted_fullhd_height")) != (
            str(TARGET_SIZE[0]),
            str(TARGET_SIZE[1]),
        ):
            issues.append(f"field16_decoder_promoted_pack_{pcx_name}_fullhd_size_mismatch")

    if total.get("review_verdict") != "field16_decoder_promoted_pack_ready":
        issues.append("field16_decoder_promoted_pack_verdict_mismatch")
    if "const TEX_LARGE_SHIFTED_2A30_FIELD16_DECODER_PROMOTED_PACK = " not in text:
        issues.append("missing_tex_large_shifted_2a30_field16_decoder_promoted_pack_json")
    ...
    return (
        gate(
            ...
            ok,
            expected="packed 4 reviewed shifted 0x2a30 field16 decoder previews as coverage-eligible Full HD assets",
            actual=f"eligible={eligible_rows}, fullhd={fullhd_assets}, issues={issue_rows}",
            evidence=f"{summary};{html_report}",
            issues=issues,
        ),
        eligible_rows if ok else 0,
    )
...
def main() -> None:
    ...
    (
        tex_large_shifted_2a30_field16_decoder_promoted_pack_gate,
        tex_large_shifted_2a30_field16_decoder_promoted_pack_eligible,
    ) = audit_tex_large_shifted_2a30_field16_decoder_promoted_pack(
        DEFAULT_TEX_LARGE_SHIFTED_2A30_FIELD16_DECODER_PROMOTED_PACK_SUMMARY,
        DEFAULT_TEX_LARGE_SHIFTED_2A30_FIELD16_DECODER_PROMOTED_PACK_MANIFEST,
        DEFAULT_TEX_LARGE_SHIFTED_2A30_FIELD16_DECODER_PROMOTED_PACK_HTML,
    )
    rows.append(tex_large_shifted_2a30_field16_decoder_promoted_pack_gate)
    (
        tex_augmented_gate,
        tex_augmented_exact_or_alias,
        tex_augmented_exact_alias_or_decoded,
        tex_augmented_exact_alias_decoded_or_raw,
        tex_augmented_field16_decoder,
        ...
        audit_tex_augmented_coverage(
            ...
            DEFAULT_TEX_AUGMENTED_FIELD16_DECODER,
    ...
    (
        tex_large_shifted_2a30_field16_small_delta_guard_review_gate,
        tex_large_shifted_2a30_field16_small_delta_guard_review_targets,
        tex_large_shifted_2a30_field16_small_delta_guard_review_remaining_exact,
        tex_large_shifted_2a30_field16_small_delta_guard_review_selected_guards,
    ) = audit_tex_large_shifted_2a30_field16_small_delta_guard_review(
        DEFAULT_TEX_LARGE_SHIFTED_2A30_FIELD16_SMALL_DELTA_GUARD_REVIEW_SUMMARY,
        DEFAULT_TEX_LARGE_SHIFTED_2A30_FIELD16_SMALL_DELTA_GUARD_REVIEW_TARGETS,
        DEFAULT_TEX_LARGE_SHIFTED_2A30_FIELD16_SMALL_DELTA_GUARD_REVIEW_SUPPORT,
        DEFAULT_TEX_LARGE_SHIFTED_2A30_FIELD16_SMALL_DELTA_GUARD_REVIEW_GUARDS,
        DEFAULT_TEX_LARGE_SHIFTED_2A30_FIELD16_SMALL_DELTA_GUARD_REVIEW_DECISIONS,
    ...
    (
        tex_large_shifted_2a30_field16_decoder_previews_gate,
        tex_large_shifted_2a30_field16_decoder_previews_native,
        tex_large_shifted_2a30_field16_decoder_previews_fullhd,
        tex_large_shifted_2a30_field16_decoder_previews_sources,
    ) = audit_tex_large_shifted_2a30_field16_decoder_previews(
        DEFAULT_TEX_LARGE_SHIFTED_2A30_FIELD16_DECODER_PREVIEWS_SUMMARY,
        DEFAULT_TEX_LARGE_SHIFTED_2A30_FIELD16_DECODER_PREVIEWS_MANIFEST,
    ...
    summary_row = {
        ...
        "tex_large_shifted_2a30_field16_decoder_promoted_pack_eligible": str(
            tex_large_shifted_2a30_field16_decoder_promoted_pack_eligible
        ),
        ...
        "tex_augmented_field16_decoder_unique_pcx": str(tex_augmented_field16_decoder),
        "tex_augmented_exact_alias_decoded_raw_or_field16_unique_pcx": str(
            tex_augmented_exact_alias_decoded_raw_or_field16
        ),
        ...
        "tex_large_shifted_2a30_field16_small_delta_guard_review_targets": str(
            tex_large_shifted_2a30_field16_small_delta_guard_review_targets
        ),
        "tex_large_shifted_2a30_field16_small_delta_guard_review_remaining_exact": str(
            tex_large_shifted_2a30_field16_small_delta_guard_review_remaining_exact
        ),
        ...
        "tex_large_shifted_2a30_field16_decoder_previews_native": str(
            tex_large_shifted_2a30_field16_decoder_previews_native
        ),
        "tex_large_shifted_2a30_field16_decoder_previews_fullhd": str(
            tex_large_shifted_2a30_field16_decoder_previews_fullhd
        ),
