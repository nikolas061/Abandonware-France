#!/usr/bin/env python3
"""Replay false-free len64 selector targets on top of the clean .tex buffers."""
...
DEFAULT_OUTPUT = Path("output/tex_gap_decoder_len64_promoted_replay")
...
DEFAULT_SELECTOR_TARGETS = Path("output/tex_gap_decoder_len64_selector_probe/targets.csv")
...
def count_mask(mask: bytes | bytearray) -> int:
    return sum(1 for value in mask if value)


def bounded_range(row: dict[str, str], size: int) -> tuple[int, int]:
    start = max(0, min(int_value(row, "expected_start"), size))
    end = max(start, min(int_value(row, "expected_end"), size))
    return start, end


def rejected_ranges(decision_rows: list[dict[str, str]]) -> dict[tuple[str, str, str], list[tuple[int, int]]]:
    ranges: dict[tuple[str, str, str], list[tuple[int, int]]] = defaultdict(list)
    for row in decision_rows:
        if row.get("queue_verdict") != "reject_false_risk":
            continue
        ranges[fixture_key(row)].append((int_value(row, "expected_start"), int_value(row, "expected_end")))
    return ranges


def overlap_bytes(mask: bytes | bytearray, start: int, end: int) -> int:
...
def build_rows(
    *,
    ...
    targets_by_fixture: dict[tuple[str, str, str], list[dict[str, str]]] = defaultdict(list)
    for row in selector_target_rows:
        if row.get("greedy_selector"):
            targets_by_fixture[fixture_key(row)].append(row)
    ...
    for clean_fixture in sorted(clean_fixture_rows, key=lambda row: fixture_sort_key(fixture_key(row))):
        ...
        decoded = bytearray(load_bytes(clean_fixture.get("decoded_path", ""), fixture_issues, "decoded"))
        known_mask = bytearray(load_bytes(clean_fixture.get("known_mask_path", ""), fixture_issues, "known_mask"))
        accepted_mask = bytearray(load_bytes(clean_fixture.get("accepted_mask_path", ""), fixture_issues, "accepted_mask"))
        ...
        if len(accepted_mask) != len(expected):
            fixture_issues.append("accepted_mask_size_mismatch")
        ...
        for target in sorted(targets_by_fixture.get(key, []), key=lambda row: (int_value(row, "expected_start"), int_value(row, "op_index"))):
            ...
            if target_length != 64:
                target_issues.append("target_not_len64")
        ...
        decoded_path = fixture_output_dir / f"{stem}_decoded_len64_promoted.bin"
        ...
        native_preview_path = native_preview_dir / f"{stem}_len64_promoted.png"
        fullhd_preview_path = fullhd_preview_dir / f"{stem}_len64_promoted_fullhd.png"
        ...
        base_clean = int_value(clean_fixture, "clean_bytes")
        rejected_false = int_value(clean_fixture, "rejected_false_bytes")
        total_clean = count_mask(known_mask)
        fixture_bytes = len(expected)
        output_fixture_rows.append(
            {
                "rank": key[0],
                "archive": manifest.get("archive", clean_fixture.get("archive", "")),
                "archive_tag": manifest.get("archive_tag", clean_fixture.get("archive_tag", "")),
                "pcx_name": key[1],
                "frontier_id": key[2],
...
<article class="card">
  ...
  <div class="card-body">
    ...
    <div class="muted">+{html.escape(row.get('selector_added_bytes', ''))} len64 - {html.escape(row.get('remaining_unresolved_bytes', ''))} unresolved</div>
...
<header>
  <div class="wrap">
    ...
    <div class="sub">Applies false-free len64 selector targets on top of the clean replay.</div>
  </div>
</header>
<main class="wrap">
  <section class="stats">
    <div class="stat"><div class="label">Added len64 bytes</div><div class="value ok">{summary['selector_added_bytes']}</div></div>
...
<script>
const TEX_GAP_DECODER_LEN64_PROMOTED_REPLAY = {data_json};
</script>
</body>
</html>
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Replay false-free len64 selector targets over clean .tex buffers.")
    ...
    parser.add_argument("--title", default="Lands of Lore II .tex Len64 Promoted Replay")
    ...
    summary, fixture_rows, promotion_rows = build_rows(
        ...
        clean_fixture_rows=read_csv(args.clean_fixtures),
    ...
    print(f"Added len64 bytes: {summary['selector_added_bytes']}")
