#!/usr/bin/env python3
"""Queue fixed-transform exact sequence matches for structured nonzero gaps."""
...
DEFAULT_OUTPUT = Path("output/tex_gap_decoder_len64_promoted_tiny_nonzero_gap_exact_sequence_probe")
...
def build_rows(
    ...
    target_rows = [row for row in pattern_rows if row.get("pattern_class") in STRUCTURED_CLASSES]
    target_bytes = 0

    for pattern in target_rows:
        issues: list[str] = []
        ...
        for pool_name, pool in pools.items():
            for transform in FIXED_TRANSFORMS:
                ...
                for offset in find_all(transformed, expected):
                    local_candidates.append(
                        {
                            ...
                            "pool": pool_name,
                            "transform": transform,
                            "match_offset": str(offset),
                            "match_end": str(offset + len(expected)),
                            ...
                            "match_hex": transformed[offset : offset + len(expected)].hex(),
                            "issues": ";".join(issues),
                        }
                    )
    ...
    summary = build_summary(target_rows, target_bytes, candidates, groups, fixture_issues)
...
def build_summary(
    ...
    exact_keys = set(target_candidates)
    exact_rows = [rows[0] for rows in target_candidates.values()]
    ambiguous = [rows for rows in target_candidates.values() if len(rows) > 1]
    tiny = [row for row in exact_rows if int_value(row, "length") < 4]
    non_tiny = [row for row in exact_rows if int_value(row, "length") >= 4]
    return {
        "scope": "total",
        "target_rows": str(len(target_rows)),
        "target_bytes": str(target_bytes),
        "exact_rows": str(len(exact_keys)),
        "exact_bytes": str(sum(int_value(row, "length") for row in exact_rows)),
        "candidate_rows": str(len(candidates)),
        "singleton_exact_rows": str(sum(1 for rows in target_candidates.values() if len(rows) == 1)),
        "ambiguous_exact_rows": str(len(ambiguous)),
        "tiny_exact_rows": str(len(tiny)),
        "non_tiny_exact_rows": str(len(non_tiny)),
        "max_exact_length": str(max([int_value(row, "length") for row in exact_rows] or [0])),
...
<header>
  <div class="wrap">
    ...
    <div class="sub">Lists fixed-transform exact sequence matches for structured nonzero gaps.</div>
  </div>
</header>
<main class="wrap">
  <section class="stats">
    <div class="stat"><div class="label">Target bytes</div><div class="value">{summary['target_bytes']}</div></div>
    <div class="stat"><div class="label">Exact bytes</div><div class="value ok">{summary['exact_bytes']}</div></div>
    <div class="stat"><div class="label">Candidates</div><div class="value">{summary['candidate_rows']}</div></div>
    <div class="stat"><div class="label">Ambiguous rows</div><div class="value warn">{summary['ambiguous_exact_rows']}</div></div>
    <div class="stat"><div class="label">Max length</div><div class="value">{summary['max_exact_length']}</div></div>
  </section>
  <section class="panel"><h2>Files</h2><div>{links}</div></section>
  <section class="panel"><h2>Pool and transform groups</h2>{render_table(groups, GROUP_FIELDNAMES, 80)}</section>
  <section class="panel"><h2>Candidates</h2>{render_table(candidates, CANDIDATE_FIELDNAMES, 280)}</section>
</main>
<script>
const TEX_GAP_DECODER_LEN64_PROMOTED_NONZERO_GAP_EXACT_SEQUENCE_PROBE = {data_json};
</script>
</body>
</html>
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Queue fixed exact sequence matches for structured .tex nonzero gaps.")
    ...
    parser.add_argument(
        "--title",
        default="Lands of Lore II .tex Tiny Nonzero Gap Exact Sequence Probe",
    )
    ...
    print(f"Exact rows: {summary['exact_rows']}")
    print(f"Exact bytes: {summary['exact_bytes']}")
